# Chapter 4 · Control: From PID to Whole-Body
...
[FIG 4.1] title: "Open-loop vs closed-loop under a disturbance"
type: toggle-compare
shows: the same single revolute joint asked to hold 90°, rendered two ways — an OPEN-LOOP version that sends a fixed command, and a CLOSED-LOOP version that measures error and corrects — with a shared "add load" disturbance
controls: two tabs (Open-loop, Closed-loop), an "add load" toggle that drops a weight on the arm, reset
interaction: with no load both hold 90°; toggling the load makes the open-loop arm sag and stay sagged (error frozen, controller oblivious), while the closed-loop arm sags for an instant then drives back up as the error is sensed and corrected
animation: the load drops with an eased sag; the closed-loop arm eases back toward 90° while a live error readout counts toward zero; the open-loop error stays pinned at its sagged value
color: blue = actual angle, amber = 90° command, grey = the dropped load, red = live error magnitude
caption: "Open-loop trusts its model and never checks; closed-loop measures the gap and erases it. The instant the world pushes back, only the loop that looks can respond."

**Check your understanding.** A toaster and a cruise-control system both "produce an output over time." One is open-loop and one is closed-loop. Which is which, and what specifically does the closed-loop one do that the open-loop one structurally cannot?
...
## Proportional control: push in proportion to how wrong you are
...
[FIG 4.2] title: "P vs PI: steady-state error appears, then dies"
type: slider-driven diagram
shows: a step response plot (angle vs time) for a joint under a constant gravity disturbance, drawn two ways — P-only converging to a line *below* the setpoint, and PI climbing the rest of the way to the setpoint — with the persistent gap shaded
controls: a `K_p` slider, a `K_i` slider that starts at zero, a "gravity" toggle, reset
interaction: with `K_i = 0` the curve settles short of the dashed setpoint and the steady-state gap is shaded red; raising `K_i` off zero visibly walks the curve up until the gap closes to nothing
animation: the response eases to its settling value; when `K_i` is raised, the tail of the curve creeps upward to meet the setpoint line
color: blue = joint angle, amber = setpoint line, red = shaded steady-state error, green = the closed gap once I is added
caption: "Proportional control against a constant disturbance always parks short of the target — it needs error to make effort. The integral term is what finally walks that last gap to zero."

**Check your understanding.** You're using pure P control to hold a drone at a target altitude, and it hovers steadily but always about 20 cm too low. A colleague says "just raise the gain." Will that work, what's the risk, and what would actually eliminate the gap?
...
## Integral control: remember the error you couldn't fix
...
**Check your understanding.** An engineer adds integral control to a temperature controller for an oven. During a test, the oven door is propped open for two minutes (so the oven can't reach temperature), then shut. When they shut the door, the oven blows way past the target temperature before settling. What happened, and what's the fix?
...
## Derivative control: watch the rate, damp the swing
...
## The full PID law: three views of the same error
...
[FIG 4.3] title: "Tuning PID: the full interactive controller"
type: slider-driven diagram
shows: a single revolute joint tracking a draggable setpoint, plus a live error-vs-time plot below it annotating overshoot, steady-state error, and settling time; a gravity disturbance can be toggled to make steady-state error real
controls: `K_p`, `K_i`, `K_d` sliders; a draggable setpoint on the arm; a "gravity" disturbance toggle; play/pause; reset
interaction: dragging the setpoint kicks off a fresh step response; sliders reshape it live — cranking `K_p` induces overshoot, adding `K_d` damps it to critically-damped, `K_i` erases the gravity-induced steady-state gap; the plot labels the current regime (underdamped / critically damped / overdamped)
animation: the arm eases toward each new setpoint; the error curve draws in real time and the annotations (overshoot peak, settling band, residual gap) update live
color: blue = joint angle, amber = setpoint, green = settling band / on-target, red = error magnitude and overshoot region, grey = the gravity load when toggled
caption: "Present, past, future — P, I, and D are three lenses on one error signal. Tuning is finding the blend that arrives fastest without ringing: critically damped, the sweet spot between twitchy and sluggish."

**Check your understanding.** A robot arm under PID control reaches its target quickly but overshoots and oscillates two or three times before settling. Which gain do you adjust and which way, and what regime is the system in? If instead it crept slowly to the target with no overshoot at all, what would you change?
...
## Feedforward: stop reacting, start predicting
...
$$u = \underbrace{u_{\text{ff}}}_{\text{feedforward: predicted}} \;+\; \underbrace{K_p e + K_i\!\int\! e\,dt + K_d \tfrac{de}{dt}}_{\text{feedback: reactive}}$$
...
[FIG 4.4] title: "Gravity compensation: predict, don't react"
type: slider-driven diagram
shows: a 2-link arm commanded to hold a pose, drawn with feedforward gravity compensation toggleable; a torque readout shows the gravity torque `m·g·L·cos(θ)` at the current pose
controls: a "gravity compensation" on/off toggle, a slider to set the arm's commanded angle, a `K_p`/`K_d` feedback-strength slider, reset
interaction: with compensation OFF the arm visibly sags below the commanded pose (feedback alone fights gravity and settles short unless gains are cranked); flipping compensation ON snaps it to the commanded pose with almost no error, and the torque readout shows the predicted gravity torque being injected
animation: toggling compensation eases the arm from its sagged pose up to the target; the torque readout tracks `cos(θ)`, peaking when the arm is horizontal
color: blue = actual arm pose, amber = commanded pose, green = the injected feedforward torque, red = residual sag error
caption: "Feedback reacts to a sag that already happened; feedforward cancels gravity before it can cause one. PD plus gravity compensation is the classic arm controller: predict the known, let feedback clean up the rest."

**Check your understanding.** Two robot arms hold the same heavy tool level against gravity. Arm A uses pure high-gain PID; arm B uses PD with a gravity-compensation feedforward term. Both hold the pose accurately. In what important way does arm B behave better, and why?
...
## Trajectory tracking: control along a whole path
...
## Force and impedance control: command a relationship, not a position
...
$$F = K(x_{\text{desired}} - x) + D(\dot{x}_{\text{desired}} - \dot{x})$$
...
[FIG 4.5] title: "Impedance control: dial the stiffness of a virtual spring"
type: draggable-field
shows: an end-effector tethered to a target rest position by a visible virtual spring; the reader can grab and drag/push the end-effector, and a stiffness slider sets how hard it springs back
controls: a stiffness `K` slider (soft ↔ rigid), a damping `D` slider, a draggable end-effector, reset
interaction: dragging the end-effector away from the rest position stretches the drawn spring and shows the restoring force `F = K·Δx` as an arrow and a number; at low `K` it barely resists and follows your hand, at high `K` it snaps back rigidly and resists being moved at all; releasing lets it spring (and damp) back to rest
animation: the spring stretches and recoils with eased, damped motion set by `K` and `D`; the force arrow scales live with displacement
color: blue = end-effector, amber = virtual rest position, green = the virtual spring, red = restoring-force arrow and magnitude
caption: "Impedance control commands a relationship, not a position: the robot behaves like a spring whose stiffness you choose. Turn the knob and the same robot goes from rigid and precise to soft and human-safe — Chapter 1's compliant actuator, now a number in software."

**Check your understanding.** A robot must insert a slightly misaligned peg into a hole. Engineer A uses a stiff position controller aimed at the hole's center. Engineer B uses low-stiffness impedance control. Why does A's peg jam or snap while B's slides in?
...
## Whole-body control: when hand-designing gets too hard
