# Chapter 3 · Kinematics: Forward, Inverse & the Jacobian
...
Now the crucial move. The second link points in the direction (θ₁ + θ₂), because θ₂ is measured relative to the already-rotated first link — the angles *add*. So from the elbow we go out another L₂ at total angle (θ₁ + θ₂), and the hand lands at:
...
[FIG 3.1] title: "Forward kinematics on a 2-link arm"
type: slider-driven diagram
shows: a 2-link planar arm anchored at the origin, with two revolute joints; a live readout panel showing L₁cosθ₁ + L₂cos(θ₁+θ₂) and the matching y-term evaluating to the current (x, y) of the hand
controls: θ₁ slider, θ₂ slider, and toggles for link lengths L₁ and L₂; reset
interaction: dragging either angle slider sweeps that joint; the elbow and hand track smoothly and the (x, y) readout recomputes term by term so the reader sees each cosine landing in the sum
animation: the arm eases to the new pose; a faint trace shows the hand's path as an angle is swept, drawing an arc
color: blue = the links and hand, grey = the base and reference axes, amber = the live (x, y) readout and equation terms
caption: "Forward kinematics is chaining links nose to tail: each term is one link pointing in the accumulated angle, and the sum is always a single, unique hand position."

**Check your understanding.** You command a 3-joint arm to a specific set of angles, run the forward-kinematics multiply, and get a hand pose. A friend insists there might be *another* hand pose consistent with those same angles. Are they right? Why does this get so much worse when you run the problem backwards?
...
## DH parameters: how to describe any arm without arguing about it

Before we go further we hit a boring-sounding problem that's secretly important: **how do you even write down an arm so someone else can reproduce it?** I've been sloppy — "link of length L," "joint turns by θ" — and for a flat 2-link toy that's fine. But a real arm has joints whose axes point in different directions, links that are offset and twisted relative to each other, wrists where three axes stab through a single point. If you and I each attach coordinate frames to the links however we feel like it, our transforms won't match, our code won't be portable, and we'll spend an afternoon arguing about whose z-axis points where.
...
[FIG 3.2] title: "Building an arm from DH parameters"
type: slider-driven diagram
shows: a 3-joint arm rendered in a light 3D-ish projection, with a 3-row DH table beside it (columns a, α, d, θ); each row's four numbers highlight the segment of the arm they control
controls: a slider for each of the four DH parameters on a currently-selected joint, plus tabs to select which joint (row) you're editing; reset to a default arm
interaction: dragging the link-length slider stretches that bone; the twist slider tilts the next joint axis out of plane; the offset slider slides along the axis; the angle slider rotates the joint — and the arm rebuilds live so the reader feels which number does what
animation: the affected link segment eases to its new geometry; the edited table cell pulses
color: grey = fixed structure, blue = the currently-selected link/joint being edited, amber = the four table values, green = the joint axes drawn as short arrows
caption: "A convention buys portability: four numbers per joint — three fixed by the metal, one free to move — describe any arm unambiguously, so anyone's software can chain the transforms."

**Check your understanding.** Of a revolute joint's four DH parameters (a, α, d, θ), which one is the *variable* the motor changes as the robot moves, and which three are constants? What flips if the joint is prismatic instead?

> Show answer ▸
>
> For a revolute joint, **θ (the joint angle)** is the variable — it's literally the rotation the motor commands — while a, α, and d are constants baked into the physical link. For a prismatic joint it flips: **d (the offset)** becomes the variable, since the joint slides along its axis, and now θ is one of the fixed constants. That's the clean payoff of the convention: every joint contributes exactly one moving number (Chapter 1's DoF-per-joint rule), and the DH table makes it explicit which number that is. Everything else in the row is geometry you measured once and never touch again, which is exactly why the description is portable.

---

## The workspace: what the arm can actually reach
...
[FIG 3.3] title: "Reachable vs dexterous workspace"
type: draggable-field
shows: the 2-link arm at the origin with two shaded regions overlaid — an outer annulus (reachable) and an inner region (dexterous where the hand can face any direction); a draggable probe point the reader moves around the plane
controls: drag the probe point; a toggle to show/hide each region; sliders for L₁ and L₂ that reshape both regions live
interaction: as the probe moves, a readout says "reachable / dexterous / unreachable"; dragging it past the outer ring turns it red; changing link lengths redraws the annulus (equal links close the inner hole entirely)
animation: the arm snaps to reach the probe when it's inside the workspace; regions ease to new shapes as lengths change
color: grey = the arm and reachable annulus, green = the dexterous core, red = the probe when it's outside all reach, amber = the readout label
caption: "The workspace is the arm's territory: a ring you can touch, with a smaller core you can touch from any angle. The edges reach but can't reorient — put the real work in the dexterous core."

---

## Inverse kinematics: the hard direction, where it all breaks
...
[FIG 3.4] title: "Inverse kinematics with elbow-up and elbow-down"
type: draggable-field
shows: the 2-link arm and a draggable target dot; when the target is reachable, BOTH IK solutions render — a solid elbow-down arm and a faint ghost elbow-up arm sharing the same hand point; a workspace ring is drawn
controls: drag the target anywhere on the plane; a toggle to prefer elbow-up vs elbow-down as the "active" (solid) solution; reset
interaction: as the target moves, both arm configurations snap to track it, visibly folding the elbow opposite ways; drag the target outside the ring and both arms turn red and freeze at full stretch to signal "unreachable — zero solutions"
animation: the two arms ease to each new target; at the outer ring they collapse to a single straight configuration (one solution) before going unreachable
color: blue = the active solution, grey ghost = the alternate elbow solution, green = the target when reachable, red = target and arm when out of reach, faint grey = workspace ring
caption: "Invert the map and it turns ill-posed: most targets have two solutions (elbow-up and elbow-down), the ring's edge has exactly one, and outside the ring there are none — the arm goes red."

**Check your understanding.** You drag the IK target of the 2-link arm slowly outward from the base toward the edge of reach. Trace what happens to the *number* of solutions along the way, and explain why the count changes at the boundary.

> Show answer ▸
>
> Near the base (but outside the tiny inner hole) the target is comfortably reachable and there are **two** solutions, elbow-up and elbow-down, the ±θ₂ from the arccosine. As you drag outward they stay at two, but the two elbow positions swing closer together — the arm is straightening. Right at the outer ring, radius L₁ + L₂, the two solutions **merge into one**: the only way to reach maximum extension is a single fully-straight configuration, so +θ₂ and −θ₂ both collapse to zero and the two branches become the same arm. Push one pixel past the ring and there are **zero** solutions — the equation's cosine argument exceeds 1, the arccosine has no real answer, and the target is simply unreachable. The count going 2 → 1 → 0 as you cross the boundary is the whole ill-posedness story in one drag, and the "1" point is a singularity, which is why IK gets numerically twitchy right at the edge.
...
**How many numbers does it take to fully specify where a rigid hand is in space?** Not just its position — its *pose*, position and orientation together, which is everything you need to grab something correctly. Count them:
...
**Check your understanding.** A pick-and-place robot only ever needs to move its gripper to an (x, y, z) position and point it straight down — the yaw around the vertical doesn't matter for the round parts it handles. How many degrees of freedom does that task actually require, and what does that let the designer do?
...
$$ J = \begin{bmatrix} -3(0.5) - 2(0.966) & -2(0.966) \\ \; 3(0.866) + 2(0.259) & \; 2(0.259) \end{bmatrix} = \begin{bmatrix} -3.43 & -1.93 \\ \; 3.12 & \; 0.52 \end{bmatrix} $$

Read a column: turning θ₂ at unit speed moves the hand at velocity (−1.93, 0.52). Turn *both* joints and you add the columns, scaled by their speeds — the matrix does the bookkeeping. That first column, (−3.43, 3.12), is exactly perpendicular to the line from the base to the hand at (3.12, 3.43) — turning the base joint whips the whole arm around, so the hand moves *sideways* to its reach, as it must. The math and the physical picture agree.

[FIG 3.5] title: "The Jacobian as a velocity ellipse"
type: draggable-field
shows: the 2-link arm plus a "manipulability ellipse" drawn at the hand — the set of hand velocities you get from spinning the joints at unit speed in all combinations; a fat ellipse means the hand moves easily in every direction, a thin one means it's stuck to a line
controls: drag the hand (which repositions the arm via IK), or drag either joint directly; a toggle to show the two column-vectors of J as arrows at the hand
interaction: as the arm moves, the ellipse stretches and rotates live; drive the arm toward full extension and the ellipse squashes flatter and flatter until it collapses to a line segment — the singularity — with a red flag
animation: the ellipse eases as the arm moves; at the singularity it visibly flattens to a line and a "rank lost" label appears
color: blue = the arm, amber = the velocity ellipse, green = the two Jacobian column arrows, red = the collapsed line and singularity warning
caption: "The Jacobian is configuration-dependent gearing: a fat ellipse means the hand moves freely in all directions; as the arm straightens the ellipse flattens, and at full stretch it collapses to a line — a lost degree of freedom."

**Check your understanding.** You solve $\dot{x} = J\dot{q}$ to make a hand move at a steady 10 cm/s in a straight line across the workspace. Why can't you compute the joint velocities once and hold them fixed for the whole trajectory?
...
## Singularities: where the gearing breaks
...
[FIG 3.6] title: "Approaching a singularity"
type: slider-driven diagram
shows: the 2-link arm with a single slider that sweeps θ₂ from folded (θ₂ = 90°) toward straight (θ₂ = 0); beside the arm, a live bar shows the manipulability number and a readout shows the joint velocity IK demands to move the hand outward at a fixed 5 cm/s
controls: θ₂ slider; a checkbox "command 5 cm/s outward"; reset
interaction: as the slider drives θ₂ toward 0, the manipulability bar drains toward zero and the demanded joint-velocity readout climbs, then spikes off the chart as the arm straightens — visually showing the inverse exploding
animation: the ellipse (reused from Fig 3.5) flattens in lockstep with the slider; the velocity readout counts up and flashes red as it diverges
color: blue = the arm, amber = the ellipse and manipulability bar, red = the diverging joint-velocity readout at the singularity
caption: "As the arm straightens, manipulability drains to zero and the joint velocity IK demands explodes — the same event seen three ways: a flattening ellipse, a vanishing manipulability, and an inverse blowing up."

---

## Redundancy: spending extra joints on a second goal
...
1. Use the ordinary Jacobian-inverse to make the hand track its task — hold the cup, follow the trajectory, whatever the six task numbers demand.
2. **Project a *second* goal into the null space** so it costs the hand nothing. Want to keep the elbow away from an obstacle? Away from a joint limit? At a comfortable, low-effort posture? Compute the joint motion that improves *that* secondary goal, then filter it through the null-space projection $(I - J^{+}J)$ so only the component that leaves the hand undisturbed survives. The elbow drifts toward safety; the cup doesn't spill.
...
[FIG 3.7] title: "Sweeping the elbow through the null space"
type: draggable-field
shows: a redundant 3-link planar arm holding its fingertip on a fixed target dot; a slider that sweeps the "self-motion" — the arm reconfigures internally, elbow tracing an arc, while the fingertip stays nailed to the point
controls: a "null-space" slider that sweeps the internal posture; drag the fingertip target to relocate it; a toggle to drop an obstacle the elbow must avoid; reset
interaction: sliding the null-space control sweeps the elbow through its arc with the fingertip perfectly fixed; with the obstacle on, an "auto" button drives the elbow through the null space to the pose that clears the obstacle, fingertip never budging
animation: the elbow eases along its arc; the fingertip dot never moves; when avoiding, the elbow smoothly retreats from the obstacle
color: blue = the arm, green = the fixed fingertip target, amber = the swept elbow / null-space motion, red = the obstacle the elbow avoids
caption: "Redundancy is a second budget: the null space of the Jacobian is the joint motion that moves nothing at the hand, so the elbow can dodge an obstacle while the fingertip stays frozen — the coffee-cup slack, spent."
