# Chapter 8 · Perception: Vision & Vision-Language Models
...
## Quick recap: an image is a grid of numbers
...
**Check your understanding.** A CNN's early filters fire on edges and its late layers fire on whole objects, yet no one ever told it what an edge or an object is. Where does that hierarchy come from, and what built-in assumption makes it efficient?
...
## Attention: the engine under everything
...
The mechanism, written once and then unpacked entirely in words:

**Attention(Q, K, V) = softmax(QKᵀ / √d) · V**

Reading the symbols left to right. **Q** is the stack of all query vectors (one row per token). **K** is the stack of all key vectors. **QKᵀ** — Q times the transpose of K — is a big table of dot products: every query dotted with every key. A dot product is a similarity score, so this table says, for each token, *how well does my question match every other token's advertisement?* Big number = strong match. **√d** (the square root of the key dimension, the length of each key vector) is just a scaling factor; without it the dot products grow large as d grows, and that pushes the next step into a saturated regime where gradients die — dividing by √d keeps the scores tame. **softmax** takes each row of scores and squashes it into a set of weights that are all positive and sum to 1 — the attention weights, literally "what fraction of my attention goes to each token." Finally we multiply those weights by **V**: each token's answer is a **weighted sum of everyone's Values**, weighted by how much attention it decided to pay. That's it. That's attention.

**A tiny worked intuition.** Say token A ("red") has a query, and there are three other tokens whose keys give raw match scores of 8, 2, and 2 after the QKᵀ / √d step. Softmax turns 8, 2, 2 into roughly 0.94, 0.03, 0.03 — the big score dominates because softmax exponentiates before normalizing. So token A's output is 0.94 of the first token's Value plus a whisper of the other two. If instead the scores were 4, 4, 4 — a tie — softmax gives 0.33, 0.33, 0.33 and A reads out a flat average of all three. The scores decide the blend; the Values decide the content. Change the question (the Query) and the whole blend re-weights.
...
[FIG 8.1] title: "Query, Key, Value — reading out a weighted sum"
type: draggable-field
shows: a row of token cards (e.g. "red", "mug", "table", "the"), each showing its Key; a movable Query token above them; below, an attention-weight bar chart and a "readout" box showing the weighted sum of Values
controls: a dropdown to pick which token is the active Query; a "temperature/√d" slider that sharpens or flattens the softmax; hover any key to see its raw dot-product score; play/pause on an auto-cycle that steps the Query through each token
interaction: hovering or selecting a Query lights up the matching Keys, the weight bars update live, and the readout box recomputes the weighted-sum-of-Values in real time; sliding √d visibly sharpens (small) or flattens (large) the weights
animation: attention weights ease into their new heights over ~400ms; the readout vector morphs smoothly as weights change
color: amber = the active Query (the command/question), blue = Keys and their match strength, green = the Value contributions that dominate the readout, grey = tokens receiving near-zero attention
caption: "Each token asks a Query, every token answers with a Key, and the matches — after softmax — decide how much of each token's Value you read out. Attention is content-addressed blending."

**Check your understanding.** In the attention formula, what breaks if you delete the softmax and just use QKᵀ · V directly, and separately, why divide by √d?
...
## Vision Transformers: an image is a sequence of patches
...
[FIG 8.2] title: "Patchify: turning an image into a sequence of tokens"
type: slider-driven diagram
shows: a sample image on the left; the same image overlaid with a patch grid that animates the chopping; on the right, the resulting row of patch tokens, each stamped with a small positional-embedding badge, feeding into a "transformer" block
controls: a patch-size selector (8 / 16 / 32), a toggle "show positional embeddings," play to animate the chop-and-flatten sequence, reset
interaction: changing the patch size re-tiles the image live and updates a readout showing patches-per-side, total tokens, and the relative attention cost (∝ tokens²); toggling positional embeddings shows what "position-blind" attention would scramble
animation: the grid lines sweep in, each patch lifts off the image and flattens into a token in the sequence, then a positional badge drops onto it — eased, ~500ms per stage
color: grey = the source image, blue = patch tokens, amber = positional embeddings, green = the readout numbers (token count, cost)
caption: "A ViT makes an image a sequence: chop into fixed patches, project each to a token, stamp it with a position, and let attention do the rest. Smaller patches see finer detail but cost quadratically more."

**Check your understanding.** You feed a ViT the same photo twice, but the second time you shuffle the patch order before adding positional embeddings — then add the *correct* positional embeddings back. Does the ViT's output change? What does that tell you about where "where things are" lives in a ViT?
...
## CLIP: forcing vision and language into one shared space
...
[FIG 8.3] title: "The shared embedding space: matches pull together, mismatches push apart"
type: draggable-field
shows: a 2D projection of the shared embedding space with dots for a few images (thumbnails) and a few captions (text chips); a similarity grid on the side highlighting the diagonal; a "before / after training" view
controls: drag an image dot near or far from a caption chip; a "train step" button that runs one contrastive update, visibly pulling the true pair together and shoving mismatches apart; a toggle between "random init" and "trained"; reset
interaction: dragging an image and hitting "train step" animates the true (image, caption) pair sliding toward each other while every mismatched pair drifts apart; the similarity grid's diagonal lights up green as training proceeds
animation: dots ease toward/away over ~600ms per train step; the grid diagonal brightens; a caption like "red mug" and its photo snap close
color: green = matching (diagonal) pairs pulled together, red = mismatched (off-diagonal) pairs pushed apart, blue = image embeddings, amber = text embeddings
caption: "CLIP trains two encoders to agree: matching image-caption pairs are pulled together and mismatches shoved apart, until a photo of a red mug and the words 'red mug' share the same neighborhood."

**Check your understanding.** CLIP is trained only to match whole images to whole captions — never told what a "cat" is, never given labeled cat boxes. Yet you can hand it a photo and the prompts "a cat / a dog / a mug" and it picks the right one. How, mechanically, does whole-image-to-whole-caption training give you category recognition it was never taught?
...
## SigLIP: the same idea, but it scales
...
[FIG 8.4] title: "CLIP's softmax-over-batch vs SigLIP's independent sigmoid pairs"
type: toggle-compare
shows: the same batch similarity grid rendered two ways — CLIP mode, where each row is normalized by a softmax across the whole row (arrows showing the row-wide coupling); SigLIP mode, where each cell is an independent green/red yes/no with no cross-cell arrows
controls: an A/B toggle (CLIP softmax vs SigLIP sigmoid); a batch-size slider; a "highlight coupling" toggle that draws the dependency links in CLIP mode and shows them vanishing in SigLIP mode
interaction: flipping to CLIP mode draws lines coupling every cell in a row (and column); shrinking the batch visibly degrades CLIP's signal while SigLIP's per-cell decisions stay intact; flipping to SigLIP mode collapses each cell to an independent yes/no
animation: coupling lines fade in/out over ~400ms on toggle; as the batch-size slider drops, CLIP's diagonal contrast visibly weakens (grid dims) while SigLIP's stays crisp
color: green = "this pair matches" (yes), red = "doesn't match" (no), amber = the softmax normalization coupling in CLIP mode, grey = the grid
caption: "CLIP normalizes each image over the whole batch of captions (a softmax that couples everything, so it needs huge batches); SigLIP scores each pair as an independent yes/no sigmoid, so it stays stable and trains well at smaller batch sizes."

**Check your understanding.** CLIP and SigLIP are both trying to build the same thing — a shared image-text space where matches are close. What specifically about CLIP's *loss* forces the giant batches, and how does SigLIP's swap remove that pressure?
...
## Vision-Language Models: giving the language model eyes
...
[FIG 8.5] title: "Anatomy of a VLM: vision encoder → projector → LLM"
type: animated pipeline
shows: three boxes in a row — VISION ENCODER (SigLIP), PROJECTOR, and LLM — with an image entering the encoder, becoming visual tokens, getting reshaped by the projector, joining a stream of text tokens (the prompt), and flowing into the LLM, which emits a text answer; a small label notes "π0 uses PaliGemma; OpenVLA uses DINOv2+SigLIP"
controls: play/pause to run the forward pass; a "prompt" selector to change the question ("What is this?" / "Point to the red mug" / "Is the left one chipped?"); a toggle to show the visual and text tokens interleaving in the LLM's input sequence
interaction: playing animates a token of data flowing through each box in turn, boxes pulse as it passes; changing the prompt changes the emitted answer; toggling shows projected visual tokens slotting in alongside text tokens in one sequence
animation: a glowing token travels encoder → projector → LLM on an eased path (~600ms per hop), each box brightening as it's reached; the output text types out
color: blue = visual tokens (the sensed signal), amber = text/prompt tokens (the command), grey = the module boxes/structure, green = the emitted answer
caption: "A VLM plugs eyes into a language model: a vision encoder makes visual tokens, a projector translates them into the LLM's space, and the LLM attends over image and words together to answer. This exact pattern is the backbone of the robot policies in Chapter 9."

**Check your understanding.** In the vision-encoder → projector → LLM pattern, the projector is often just a couple of small linear layers — tiny compared to the encoder and the LLM. Why is such a small piece the thing that makes the whole system work, and what would go wrong if you deleted it and fed the raw vision tokens straight to the LLM?
...
## Generative modeling primer: diffusion and flow matching
...
[FIG 8.6] title: "Diffusion vs flow matching: many wiggly steps vs a few near-straight ones"
type: toggle-compare
shows: two side-by-side panels, each transporting a cloud of noise (left) to a data distribution (right — say a simple 2D shape or a little trajectory). Diffusion panel: a sample takes many small, wiggly, jittering denoising steps. Flow-matching panel: the same sample rides a near-straight arrow field in a few steps. A step-count readout under each.
controls: an A/B toggle (Diffusion / Flow matching); a step-count slider; a "show velocity field" toggle that draws the flow-matching arrows; play to animate a sample traveling noise→data; reset
interaction: dragging the step-count slider down visibly breaks diffusion (the sample lands off-target with too few steps) while flow matching still arrives cleanly with just a few; toggling the velocity field reveals the near-straight optimal-transport arrows guiding the flow
animation: in diffusion mode the sample stutters along a jittery path over many frames; in flow-matching mode it glides along a straight arrow in a few eased hops; both end on the green target
color: red = pure noise (the starting static), grey = the intermediate path, green = the data target/arrival, amber = the flow-matching velocity arrows, blue = the moving sample
caption: "Both turn noise into data, but diffusion crawls there over many small stochastic denoising steps while flow matching rides a near-straight learned velocity field in a few — which is why robots that must act fast generate their motions with flow matching."

**Check your understanding.** Both diffusion and flow matching turn noise into data and both can capture the same rich, multi-modal distributions. Why, then, does a real-time robot policy overwhelmingly prefer flow matching, and what property of the *paths* is doing the work?
...
**Future reference:** The *ideas* here are the stable core and won't age fast — attention, patchified vision, contrastive shared spaces, the encoder→projector→LLM pattern, and the diffusion/flow-matching split are load-bearing across the whole modern stack. The *specific models* are a moving snapshot: SigLIP, PaliGemma, DINOv2, and the exact encoder used by π0 or OpenVLA will be superseded — read those as "the state of the art the year this was written," and expect the names to churn while the pattern holds. Come back to Fig 8.1 (Q/K/V attention) whenever a later architecture stops making sense — nearly every model in Chapters 9 through 11 is that one mechanism, rearranged. And keep Fig 8.6 (flow matching) close; the moment Chapter 9 starts generating actions, it's this figure with joint angles on the target side.
