# Chapter 7 · Reinforcement Learning for Robots
...
## When to reach for RL (and when not to)
...
[FIG 7.1] title: "When the model runs out"
type: toggle-compare
shows: three task cards — "arm pick-and-place," "quadruped on rubble," "in-hand reorientation" — each with a small gauge showing "how well do we know the dynamics?" and a recommendation (classical control / RL / RL) that lights up
controls: three tabs (one per task); a toggle "show why" that reveals the one-line reason (clean rigid-body dynamics vs unpredictable contact vs discontinuous multi-contact)
interaction: selecting a task fills the model-quality gauge and highlights the recommended approach; "show why" explains the call
animation: the gauge needle eases to its value; the recommended box pulses once
color: green = model well-known (favor classical), red = model unknown/too complex (favor RL), grey = the task structure, amber = the recommendation
caption: "RL isn't better than classical control — it's the tool for when the model runs out. The worse you know the dynamics, the more learning from consequences is the only option."

**Check your understanding.** You're building a robot to screw caps onto bottles on an assembly line — same bottle, same cap, same position, every time. Your colleague wants to train it with RL "because RL is state of the art." What's your objection?
...
## The MDP: putting the problem in a box
...
- **Transition dynamics, P.** The rule for how the world changes: given state `s` and action `a`, which state `s'` do you land in? Written `P(s' | s, a)` — "the probability of `s'` given you did `a` in `s`." This is the physics, and crucially, **in the hard problems we don't get to know P.** That's the whole point. If we knew P exactly, we'd use Chapter 4's control.
...
The loop runs like this, forever: the robot observes state `s_t`, picks an action `a_t` according to its **policy** (its behavior — the thing we're trying to learn, written `π`), the environment transitions to `s_{t+1}` and hands back reward `r_t`. Sense, plan, act, repeat — exactly Fig 1.1, now with a scorekeeper attached.
...
> **G = r₀ + γ·r₁ + γ²·r₂ + γ³·r₃ + …  =  Σ γᵗ · rₜ**

Reading the symbols:

- **G** is the **return** — the whole point, the number we want to make big. It's the sum of all future rewards from now on.
- **rₜ** is the reward received at timestep `t`. `r₀` is right now, `r₁` is next step, and so on.
- **γ** (gamma) is the **discount factor**, between 0 and 1. Notice it's raised to the power `t`, so it shrinks as we look further ahead: a reward 10 steps away is worth only `γ¹⁰` of a reward right now.
...
Worked example. Say a robot takes three steps and gets rewards `r₀ = 1`, `r₁ = 1`, `r₂ = 1` (three good steps), with γ = 0.9. The return from the start is `G = 1 + 0.9·1 + 0.9²·1 = 1 + 0.9 + 0.81 = 2.71`. Now suppose the same three good steps but then it falls, `r₃ = −10`. The return becomes `2.71 + 0.9³·(−10) = 2.71 − 7.29 = −4.58`. The fall, three steps in the future, drags the whole return negative — *and it should*, because those three "good" steps led somewhere bad. Hold that thought. Making the return reflect a future consequence is exactly the credit-assignment magic we're building toward.

[FIG 7.2] title: "The agent–environment loop"
type: animated pipeline
shows: two boxes — AGENT (containing the policy π) and ENVIRONMENT (containing the hidden dynamics P) — connected in a cycle; a data token flows agent→(action a)→environment→(state s' and reward r)→agent
controls: play/pause, speed slider, and a "discount γ" slider that visually shows future rewards shrinking as γ drops
interaction: dragging γ toward 0 makes the far-future reward tokens fade to nearly nothing; toward 1 keeps them bright, illustrating far-sighted vs shortsighted
animation: the token circulates state→action→reward→state on an eased loop; a running "return G" accumulator ticks up as rewards arrive, each new reward pre-multiplied by the current γᵗ
color: blue = state (sensed signal), amber = action (the command/plan), green = reward, grey = the environment and its hidden dynamics
caption: "The MDP is Chapter 1's sense–plan–act loop with a scorekeeper. The agent never sees the dynamics P; it only sees the states and rewards the world hands back, and its job is to maximize the discounted sum of those rewards."

**Check your understanding.** Two robots learn the same task, one with γ = 0.5 and one with γ = 0.99. A shortcut gives a small reward now but leads to a dead end; the long way gives no reward for many steps but a big reward at the end. Which robot takes which route, and why?

> Show answer ▸
>
> The γ = 0.5 robot takes the shortcut. With a discount of 0.5, a reward `n` steps away is worth only `0.5ⁿ` — after ten steps that's about a thousandth of its face value, so the big delayed reward is almost invisible to it, and the immediate small reward dominates its return. The γ = 0.99 robot takes the long way: at 0.99, even a reward twenty steps out retains about 80% of its value, so the big payoff at the end outweighs the small one now. This is the discount factor doing its job — it literally sets how far into the future the robot is willing to look, and choosing it is one of the real decisions in setting up an RL problem. Too low and the robot is myopic; too high and training becomes unstable because tiny value errors compound over the long horizon.

---

## Value functions: how a delayed reward flows backward
...
> **V(s) = expected value of G, starting from state s**
...
Reading the symbols: `V(s)` is the value of where you are, `r` is the reward you collect on this step, `γ` is the discount, and `V(s′)` is the value of the next state you transition into. (The full equation averages over all the next-states the dynamics might produce and all the actions your policy might take, but this stripped-down version carries the whole intuition.) The value of *here* is defined in terms of the value of *there* — value is a chain, each link pinned to the next.
...
Start at the Goal side and work backward, which is exactly what Bellman lets us do. Cell C is one step from the Goal: `V(C) = 0 + 0.9 · V(Goal)`. Take `V(Goal) = 10`. So `V(C) = 0.9 · 10 = 9`. Cell B: `V(B) = 0 + 0.9 · V(C) = 0.9 · 9 = 8.1`. Cell A: `V(A) = 0.9 · V(B) = 0.9 · 8.1 = 7.29`. Look at what happened: **the +10 at the goal, which the robot doesn't see until the very end, has painted a gradient of value all the way back down the corridor** — 7.29, 8.1, 9, 10 — each cell knowing it's a step closer to the payoff, *even though every one of those cells hands out a reward of zero.* That gradient is what lets the robot know which way to go before it ever reaches the reward. Sparse reward, solved, by value flowing backward.

[FIG 7.3] title: "Value iteration coloring a gridworld"
type: stepper
shows: a small gridworld with a green goal cell (reward +10) and some walls; every cell starts grey (value 0); on each step, the Bellman update runs once and cells recolor by their new value, the color spreading backward from the goal like a stain
controls: step (▶ apply one Bellman sweep), play/pause (auto-step), reset; a γ slider; step counter "k / N"
interaction: each step applies one round of V(s) ← r + γ·max V(s′) across all cells; the reader watches the value ripple outward from the goal; lowering γ makes the far cells stay dark (distant reward barely reaches them)
animation: on each sweep, updated cells ease from their old color to their new value color; a small number in each cell shows its current V
color: green = high value (near goal), fading through amber to grey = low value (far), red outline = walls/traps
caption: "Watch credit assignment happen: one reward at the goal spreads backward through the Bellman equation, painting every cell with how-good-is-it-to-be-here — even cells that never give any reward themselves."

**Check your understanding.** In the corridor example, we computed `V(A) = 7.29` even though standing in cell A earns a reward of exactly zero. In plain English, what does that 7.29 represent, and why isn't it zero?

> Show answer ▸
>
> The 7.29 is the *expected future return* from cell A — not the reward you get *at* A (which is zero), but the total discounted reward you can expect to collect *from A onward* by following the policy. Cell A is three steps from a +10 goal, so its value is `10` discounted by `γ³ = 0.9³ ≈ 0.729`, giving 7.29. The whole point of a value function is that it looks past the immediate reward to the future the state leads to, which is exactly why a cell handing out zero reward can still be valuable: it's on the road to something good. This is how RL solves sparse, delayed reward — the Bellman recursion propagates the distant payoff backward until even far-off states know which way leads home.

---

## Two ways to learn, and the explore–exploit dilemma
...
**Value-based methods** learn a value function — usually Q(s, a) — and then act by always picking the highest-value action. The policy is *implicit*: it's just "look up Q for every action here, do the best one." You never represent the behavior directly; you represent *how good things are* and let that dictate the behavior. The classic here is **Q-learning** (Watkins, 1989): repeatedly nudge your estimate of `Q(s, a)` toward `r + γ · max Q(s′, a′)` — the Bellman equation again, now used as a learning rule. Do that enough and Q converges to the truth, and greedy action on the true Q is optimal. Q-learning powered the famous Atari-from-pixels result (DQN, Mnih et al., 2015).

**Policy-based methods** skip the value function as the end product and learn the behavior *directly* — a policy `π_θ(a | s)`, a function with parameters `θ` (a neural net's weights) that takes a state and outputs an action (or a probability over actions). You then adjust `θ` to make good actions more likely. We'll spend the rest of the chapter here, because for robots with continuous actions — "set this joint to 1.37 radians," not "move left or right" — policy methods are the natural fit, and they're what actually runs on legged robots.
...
**Check your understanding.** A delivery robot has learned that Route A takes 10 minutes and always uses it. Route B, which it tried once early on when a road happened to be closed, took 25 minutes, so it never tries B again. Why might pure exploitation be leaving reward on the table, and how does ε-greedy help?
...
## Policy gradients: optimize the behavior directly

Let's commit to the policy-based path, because it's where robots live. We have a policy `π_θ(a | s)` — a neural net with weights `θ` that reads the state and outputs a distribution over actions. We want to find the `θ` that maximizes expected return. The question is: **which direction should we nudge `θ`?**
...
- **∇θ J** is the gradient of the objective `J` (expected return) with respect to the parameters — the direction to step `θ` to increase return. We climb it.
- **E[…]** means "average over many rollouts" — we can't compute this exactly, so we estimate it by sampling episodes and averaging.
- **∇θ log π_θ(a | s)** is the gradient of the log-probability of the action we actually took. Intuitively, it's the direction in parameter space that would make *this action, in this state, more likely.* Push `θ` this way and next time you're in `s`, you'll pick `a` a bit more often.
- **G** is the return that followed — the same discounted return from before. It's the *weight* on the update.

Put the pieces together and the mechanism is gorgeous. For each action you took, you compute "the direction that makes this action more likely" and then **scale it by how well the episode went.** If `G` is big and positive, you take a big step toward making that action more likely — reinforce it. If `G` is negative, the sign flips and you step *away* — suppress it. Actions from good episodes get pushed up, actions from bad episodes get pushed down, all automatically, just by multiplying the "make-it-likelier" direction by the return. The robot bootstraps a good policy out of nothing but "try stuff, keep what worked."

Worked example, kept tiny. A robot in some state can go **left** or **right**, currently 50/50. It runs ten episodes. The five where it went right averaged a return of +8; the five where it went left averaged −2. REINFORCE takes the "make-right-more-likely" direction and scales it by +8 (a strong push up), and the "make-left-more-likely" direction scaled by −2 (a push *down*, because of the negative sign). Net effect: after the update, `π_θ(right | s)` climbs above 50% and `π_θ(left | s)` drops. The policy has learned, from raw experience and nothing else, that right is better here. Multiply that across millions of states and you have a walking robot.
...
[FIG 7.4] title: "Reinforcing good trajectories, suppressing bad"
type: slider-driven diagram
shows: a start point spawning several trajectory rollouts fanning out across a field toward a goal region; each trajectory is colored by its return; after an update, the policy's "preferred direction" arrow shifts toward the high-return trajectories and away from the low-return ones
controls: a "sample new rollouts" button (re-rolls trajectories), an "apply update" button (shifts the policy), a "variance" slider that spreads the returns more or less
interaction: applying an update visibly bends the policy toward the green (high-return) trajectories; cranking variance up shows the update direction jitter wildly between samples — the fatal-flaw demonstration
animation: trajectories draw in with eased strokes; the policy arrow rotates smoothly toward the reinforced region on update
color: green = high-return trajectory (reinforced), red = low-return trajectory (suppressed), amber = the policy's current preferred action direction, grey = the field
caption: "Policy gradients push up the probability of whatever led to high return and push down the rest. It works — but because it weights every action by the whole noisy episode return, the update direction jitters violently, which is REINFORCE's fatal high-variance flaw."

**Check your understanding.** In an episode, a robot makes a genuinely great action at step 3, then a terrible one at step 20 that causes a low overall return. What does plain REINFORCE do to the great step-3 action, and why is that a problem?
...
## Actor–critic: judge each action, don't just tally the episode
...
- The **actor** is the policy `π_θ` — it chooses actions. It's the thing we ultimately want.
...
[FIG 7.5] title: "Advantage: how much better than average?"
type: slider-driven diagram
shows: a bar chart for one state — a horizontal "baseline" line at V(s), and bars for several candidate actions at their Q(s,a) values; the advantage of each action is drawn as the gap above or below the baseline
controls: a slider that raises/lowers the baseline V(s); a slider for the chosen action's Q; a toggle "REINFORCE view" that colors bars by raw return G instead of advantage, so the reader sees how much noisier that is
interaction: moving the baseline flips actions from positive to negative advantage (reinforced ↔ suppressed) in real time; toggling to REINFORCE view shows the bars all shifting by the same large offset — the variance the baseline removes
color: green = positive advantage (above baseline, reinforce), red = negative advantage (below, suppress), grey = the baseline V(s), amber = the currently chosen action
caption: "The critic supplies a baseline — the average value of the state — and the actor updates on the advantage, the gap above or below it. Subtracting the baseline cancels the luck that's common to the whole state, which is how actor–critic tames policy-gradient variance."

**Check your understanding.** Two states, X and Y. In X, every action returns about 100 (it's just a great place to be). In Y, every action returns about 0. In state X you take an action that returns 101. Should the policy strongly reinforce that action? What does the advantage say versus the raw return?

> Show answer ▸
>
> No — it should barely nudge it. The raw return is 101, which plain REINFORCE would treat as a huge signal to reinforce the action strongly. But that 101 is almost entirely due to *being in state X*, not to the action you chose — every action there returns about 100. The advantage exposes this: with `V(X) ≈ 100`, the action's advantage is `101 − 100 = +1`, tiny. It was only marginally better than average, so it deserves only a marginal push. This is exactly the noise the baseline removes: it strips out the value that comes from the *situation* and isolates the value that comes from the *action*, so the policy learns from what it actually controlled rather than from the luck of where it happened to be.

---

## PPO: the update that refuses to blow up
...
A ratio of 1 means the policy hasn't changed on this action; 1.3 means the new policy is 30% more likely to take it; 0.7 means 30% less likely. The plain policy-gradient objective is basically `ratio · A` — push the ratio up when the advantage is positive, down when it's negative. The danger is that `ratio` can run away — the update makes a good action *wildly* more likely in one step, far past where the gradient was trustworthy.

PPO's answer is the **clipped surrogate objective.** In words: **let the ratio help you, but only up to a limit — clip it so the policy can't move too far in one step.** Formally you take the *smaller* of the raw objective and a version where the ratio is clipped to a band around 1 (typically `[1−ε, 1+ε]` with ε ≈ 0.2):

> **objective  =  min( ratio · A ,  clip(ratio, 1−ε, 1+ε) · A )**

Here's the intuition, which is the part that matters. Suppose the advantage is **positive** — a good action, you want to make it more likely. The objective rewards raising the ratio... but only until the ratio hits `1+ε`. Past that, the clipped term takes over and **flattens out** — pushing the ratio higher yields *no more objective*, so the gradient goes to zero and the update stops driving the policy further in that direction. You get the improvement, then the door closes. Now suppose the advantage is **negative** — a bad action, you want to make it less likely. Symmetrically, the objective lets you lower the ratio until it hits `1−ε`, then flattens. Either way, **the objective is flat outside the trust region, so no single update can shove the policy more than a bounded amount from where it started.** The `min` is what makes it safe in both directions — it always takes the more conservative (less optimistic) of the two terms, so PPO never rewards moving too far.
...
[FIG 7.6] title: "PPO clipping: the trust region"
type: slider-driven diagram
shows: a plot of the PPO objective (y-axis) against the probability ratio (x-axis); the raw objective is a straight line, the clipped objective bends flat outside the [1−ε, 1+ε] band; the clip region is shaded
controls: a slider for the probability ratio (drag it left/right and watch the objective); a slider for ε (widens/narrows the trust region); a toggle for the sign of the advantage (positive ↔ negative), which flips which side flattens
controls-note: an "advantage sign" toggle is the key control — it shows the objective flattening on the up-side for positive A and the down-side for negative A
interaction: dragging the ratio past 1+ε (with positive advantage) shows the objective going flat and the gradient dying — the reader feels the door close; flipping the advantage sign mirrors the whole picture
animation: the clipped-objective curve redraws smoothly as ε changes; a marker rides along it as the ratio slider moves
color: blue = the probability ratio (the thing under control), amber = the objective value, green = inside the trust region (safe to move), red = outside it (clipped, gradient zero)
caption: "PPO lets the policy improve, then slams a soft ceiling on how far it can move in one step: outside the clip band the objective goes flat, the gradient dies, and the update can't shove the policy off a cliff. That one trick is why PPO is stable enough to train thousands of robots in parallel."

**Check your understanding.** PPO clips the probability ratio to a band like [0.8, 1.2]. Suppose an action has a large positive advantage, and the new policy already makes it 1.2× as likely as the old one did. What happens if the gradient wants to push the ratio to 1.5, and why is that the desired behavior?
...
## Sim-to-real: training in a world that isn't quite ours
...
[FIG 7.7] title: "Domain randomization"
type: slider-driven diagram
shows: a single robot on terrain, with a panel of physics parameters (friction, mass, motor strength, latency, terrain roughness) shown as bars; a "new episode" action re-rolls every parameter to a new random value within its range, and the robot's appearance/terrain updates to match; a "reality" marker sits somewhere inside each parameter's range
controls: a "roll new episode" button (re-randomizes all parameters), a "randomization width" slider (narrow → wide), a toggle "show reality marker"
interaction: rolling new episodes visibly scatters the parameters; widening the randomization band makes reality's marker sit comfortably inside every range (robust transfer), while a too-narrow band leaves reality outside the training range (the reality gap, shown red)
animation: parameter bars ease to new random values each roll; the robot/terrain re-skin to reflect the new physics
color: grey = the varying world parameters, blue = the robot/policy, green = reality falls inside the trained range (transfers), red = reality falls outside it (reality gap)
caption: "Don't match reality — outrun it. By randomizing masses, friction, latency, and terrain every episode, the policy is forced to handle a whole distribution of physics, and the real world becomes just one more variation it already knows how to survive."

**Check your understanding.** A team trains a walking policy in a simulator they've calibrated *extremely* carefully to match their specific robot — same measured masses, same measured friction — with no randomization. It walks perfectly in sim. Why might it still fail on the real robot, and how would domain randomization have helped?
...
## Teacher–student: learning with information the real robot won't have
...
[FIG 7.8] title: "Teacher–student distillation"
type: animated pipeline
shows: two stacked pipelines — TOP (teacher): inputs include onboard sensors PLUS privileged info (true friction, terrain height, contact states, payload mass) → teacher policy → action; BOTTOM (student): inputs are ONLY a history of onboard sensors → student policy → action; an arrow labeled "imitate / distill" connects the teacher's action down to the student, which is trained to match it
controls: a toggle "reveal privileged inputs" (highlights the extra inputs only the teacher gets, in green), a "deploy to real robot" button that greys out the entire privileged-info channel to show what vanishes at deployment, a step control walking through: train teacher → distill → deploy student
interaction: hitting "deploy" removes the privileged inputs and shows only the student surviving onto the real robot; stepping through shows the teacher's behavior transferring into the student that runs on sensors alone
animation: the privileged-info channel glows during teacher training, then fades out on deploy; the student pipeline lights up as the surviving, deployable path
color: green = privileged information (sim-only, ground truth), blue = onboard sensors (available on the real robot), amber = the action/command, grey = the parts that don't deploy
caption: "Cheat in training so you don't have to cheat at deployment: a teacher learns fast using privileged ground truth the real robot can't measure, then hands its behavior to a student that runs on onboard sensors alone. This is how ANYmal learned to walk over terrain it had never seen — and why robustness to kicks, payloads, and ice falls out for free."

**Check your understanding.** Why not just train the student directly with RL on onboard sensors, and skip the teacher entirely? What does the teacher stage actually buy you?
