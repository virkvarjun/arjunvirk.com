# Chapter 12 · Manipulation, Humanoids & Deploying Real Systems
...
## Grasping: the problem that refuses to be solved
...
### The classical view: force closure and antipodal grasps
...
### The learned view: predict the grasp straight from a picture
...
[FIG 12.1] title: "Finding an antipodal grasp"
type: draggable-field
shows: a 2D object (an irregular blob or mug cross-section) the reader can drag/rotate, with a parallel-jaw gripper that attempts a grasp; the two contact points are shown with their surface-normal arrows, and a readout states whether the normals oppose (force closure ✓) or not (grasp fails ✗)
controls: drag the object to reposition/rotate it, a slider for the gripper's approach angle, a toggle "show friction cone," reset
interaction: as the object rotates, the two contact points and their normals update live; when the line between contacts aligns with opposing normals inside the friction cones, the readout flips to "force closure — holds," otherwise "will slip"
animation: the gripper fingers close with eased motion onto the two contact points; on a failing grasp the object visibly slips/rotates out of the grip
color: blue = the object, amber = the gripper and approach line, green = a valid force-closure grasp (opposing normals), red = a failing grasp / slip direction, grey = friction cones
caption: "An antipodal grasp works when two opposing contact points have surface normals pointing at each other inside their friction cones — then the fingers can resist any disturbance. Rotate the object and watch a holding grasp become a slipping one; that fragility is why grasping isn't solved."

**Check your understanding.** A classical force-closure planner needs a CAD model of the object to compute contact normals and a quality metric. A warehouse robot faces a bin of thousands of never-before-seen products. How does the learned approach (Dex-Net / GraspNet) get around the missing model, and where did its physics knowledge come from?
...
## Dexterous manipulation: reorienting the object inside the hand
...
[FIG 12.2] title: "Reorienting an object in-hand"
type: stepper
shows: a top-down or 3D-ish view of a multi-finger hand holding a cube; a target orientation is shown ghosted; stepping through the sequence, fingers make, break, and slide contacts to rotate the cube toward the target, one contact-switch per step
controls: play/pause, step (advance one contact-switch), reset, a slider for target orientation, a toggle "show contact forces"
interaction: each step advances the manipulation; the contact points visibly detach and reattach; the cube's orientation eases toward the ghosted target; stepping shows k/N contact-switches
animation: fingers reposition with eased motion between contact states; the cube rotates incrementally; contact-force arrows pulse where fingers press
color: blue = the object/cube, amber = the target orientation (ghosted), green = active gripping contact, red = a broken/sliding contact (the risky moment), grey = the hand structure
caption: "In-hand manipulation reorients an object by making, breaking, and sliding contacts in sequence — each switch is a discontinuous change in the dynamics. That contact-rich, unwritable-by-hand quality is exactly why it took reinforcement learning (OpenAI's Dactyl, 2019) and why it's still a frontier."

**Check your understanding.** Why is in-hand reorientation a poster child for reinforcement learning specifically, rather than the classical control and planning of Chapters 4 and 6? And what does vision-based tactile sensing (GelSight) add that a wrist force/torque sensor doesn't?
...
## Whole-body control and humanoids: the maximal integration challenge
...
[FIG 12.3] title: "Which chapter runs which body part?"
type: toggle-compare
shows: a line-art humanoid; clicking a body region highlights it and shows which chapter's method controls it — legs = RL locomotion (Ch7), arms/hands = VLA manipulation (Ch9), torso/whole-body = balance control (Ch4, ZMP), head/eyes = perception (Ch8), and the whole body = state estimation (Ch5)
controls: clickable body regions (legs, arms, torso, head), plus a "show all" tab that labels every region at once
interaction: clicking a region reveals a callout naming the method, the chapter, and one sentence on why that tool fits that part; "show all" overlays the full subsystem map
animation: the selected region highlights with an eased glow; a small characteristic idle motion per region (legs shift weight, arm sweeps) — calm, looping, reduced-motion-safe
color: blue = balance/whole-body control (Ch4), amber = VLA manipulation (Ch9), green = RL locomotion (Ch7), grey = structure, with perception (Ch8) and estimation (Ch5) called out on the head and body
caption: "A humanoid inherits every problem in this guide at once, and the modern answer is to factor it: RL runs the legs, VLAs run the hands, balance control mediates the whole body, perception feeds the head. Click a region to see which chapter's method lives there."

**Check your understanding.** Explain the ZMP in one sentence, and then explain why modern humanoids don't try to solve balance *and* manipulation with a single monolithic policy.
...
## Putting the stack together: the full deployment pipeline
...
1. **Perception (Chapters 5, 8).** Cameras and depth sensors stream in; a vision model segments the scene, finds the cup, estimates its pose; state estimation tracks where the robot itself is. The loop's *sense* beat.
2. **High-level reasoning / planning (Chapters 6, 9).** A vision-language model or VLA interprets the instruction, grounds "the red cup" to the detected object, and decides the sequence: approach, grasp, lift, carry, release. The *plan* beat, now semantic.
3. **Low-level control (Chapter 4).** The chosen grasp pose becomes joint targets; a controller drives the arm there and closes the gripper with compliant force control so it doesn't crush the cup. The *act* beat.
4. **Back to sense.** The action moved the world (and the cup), so the loop closes and re-perceives — did the grasp hold? Correct and continue.
...
### ROS 2: the glue that makes it a system
...
[FIG 12.4] title: "The pub/sub graph of a real robot"
type: animated pipeline
shows: a ROS 2 node graph — nodes (camera, perception, planner/VLA, controller, gripper, TF) as labeled boxes, connected by directed arrows labeled with topic names (/camera/image, /objects, /goal_pose, /joint_cmd, /tf); a data token flows through the graph tracing one pass of the sense-plan-act loop
controls: play/pause, speed slider, a toggle "highlight sense/plan/act phases" that color-groups the nodes by loop beat, a toggle "kill a node" that drops a node and shows downstream subscribers stop receiving data
interaction: playing sends a data token from camera → perception → planner → controller → gripper along the topic arrows, lighting each node as it passes; killing a node shows the pub/sub decoupling — only its subscribers go dark, the rest keep running
animation: the token travels along topic arrows with eased motion; nodes pulse as they publish; the TF node quietly pulses in the background publishing transforms
color: blue = sensed data flowing (sense), amber = plan/command topics (plan), green = control/actuation (act), grey = the nodes and idle topics, red = a killed node and its starved subscribers
caption: "A deployed robot is a graph of nodes publishing and subscribing on named topics — the sense-plan-act loop made concrete in ROS 2. Because nodes agree only on topic names, not on each other, you can swap or kill one and the rest keep running; that decoupling is what turns research demos into an assemblable system."

**Check your understanding.** In a ROS 2 system, the perception node publishes to `/objects` and the planner subscribes to `/objects`. You want to replace the perception node with a better model. Why can you do that without modifying the planner — and what's the one thing you *must* keep the same?
...
## Evaluation: the problem nobody wants to talk about
...
[FIG 12.5] title: "The generalization gap: seen vs unseen"
type: slider-driven diagram
shows: two success-rate distributions side by side — one over "seen" conditions (trained objects/environments) and one over "unseen" (novel objects/environments) — drawn as histograms or violin plots, with the mean success rate marked on each and the gap between them highlighted
controls: a "trials per condition" slider (few → many) that visibly widens/narrows the confidence bands, a slider for "how novel are the unseen conditions," a toggle "show confidence interval," reset
interaction: lowering the trial count fattens the uncertainty bands until the seen/unseen difference is statistically meaningless (the small-sample problem made visible); increasing novelty pushes the unseen distribution down and widens the gap
animation: the distributions and their means ease as sliders move; the confidence bands breathe wider/narrower with trial count
color: green = seen-condition success (the flattering number), amber = unseen-condition success (the honest number), red = the generalization gap and the noise band when trials are too few, grey = axes
caption: "A robot's success rate on trained conditions flatters; the gap to unseen conditions is the honest measure of whether it learned or memorized. Drop the trial count and watch the difference dissolve into noise — real-world evaluation is expensive, thin, and easy to fool, which is why it's one of the field's biggest open problems."

**Check your understanding.** A vendor reports "92% success rate" for their manipulation robot. List three questions you'd need answered before that number means anything, and explain why the seen-versus-unseen gap is often more informative than the headline success rate.
...
## Safety and deployment: the layer that isn't in the highlight reel
...
[FIG 12.6] title: "Stiff vs compliant: what happens when a human is in the way"
type: toggle-compare
shows: the same robot arm moving toward a target with a human hand placed in its path, rendered two ways — a stiff/position-controlled arm and a compliant/force-limited arm; the reader watches each encounter the hand
controls: two tabs (Stiff, Compliant), a "force limit" slider (compliant mode), a play button that runs the arm into the obstacle, reset
interaction: in Stiff mode the arm drives through, force spiking past the safe limit (shown in red — the danger); in Compliant mode the arm meets the hand, the sensed force hits the limit, and the arm yields/stops safely (green); the force-limit slider sets where compliant mode gives way
animation: the arm moves in with eased motion; on contact, a live force gauge climbs — spiking through the red danger zone for stiff, halting at the green limit for compliant
color: blue = the robot arm, amber = its target/command, green = safe yielding (compliant, force under limit), red = the force spike / danger (stiff drives through), grey = the human hand and structure
caption: "A robot sharing space with people must be able to yield: a stiff position-controlled arm drives through an obstacle and spikes the contact force into the danger zone, while a compliant force-limited arm senses the resistance and stops safely under its force cap. That single property — the ability to give way — is the foundation collaborative-robot safety standards (ISO/TS 15066) are built on."

**Check your understanding.** A humanoid completes a flawless demo folding laundry in a lab. Name three things that still stand between that demo and deploying the same robot into people's homes, and connect at least one of them back to the stiff/compliant idea from Chapter 1.
