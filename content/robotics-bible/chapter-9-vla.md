...
## What a VLA actually is — and the one design question
...
1. **Discrete action tokens.** Chop each action dimension into, say, 256 bins and treat each bin as a "word." Now actions *are* tokens and the language model needs no surgery at all. Dead simple. It's how RT-1, RT-2, and OpenVLA work — and its cracks are exactly what motivates everything after.
2. **Compressed tokens (FAST).** Keep the token idea, but don't tokenize raw actions — first transform them into the *frequency domain*, throw away the parts you can't perceive, and tokenize what's left. Ten times fewer tokens, same fidelity.
3. **Continuous action heads (diffusion / flow).** Drop the discrete pretense entirely. Bolt a small generative head — a diffusion or flow-matching model — onto the backbone and let it *generate* a continuous action vector directly. This is Octo and π0.
...
[FIG 9.1] title: "A VLA forward pass"
type: animated pipeline
shows: four stages left to right — an input panel (a camera image of a tabletop plus the text "pick up the mug"), a VLM backbone box, an action-head box, and a robot arm on the right; a glowing token of data flows through each stage lighting it in turn, ending with the arm moving to grasp the mug
controls: play/pause, a step button that advances one stage at a time with a caption for each, and a toggle "swap action head" that cycles the head between "discrete tokens," "FAST tokens," and "flow" so the reader sees the same pipeline with three different backs
interaction: stepping through pauses on each stage and names what it computes (encode image+text → fuse into features → head emits action → robot executes); swapping the head changes only the third box and its output representation, driving home that the backbone is shared
animation: the data token eases stage to stage on a smooth path; the final arm motion is a short calm grasp; boxes pulse as the token passes
color: blue = the sensed image/features (the signal), amber = the language instruction and the emitted action (the command), grey = the model structure, green = successful grasp
caption: "Every VLA is one pipeline: image and instruction in, a shared VLM backbone in the middle, an action head on the end. The three action-head choices are the whole design space."

**Check your understanding.** A vision-language model from Chapter 8 already takes an image and an instruction and outputs text. What is the *minimum* change needed to turn it into a VLA, and why is that change deceptively deep?
...
## RT-1: a body without a brain (2022)
...
And here's the move that echoes through the whole chapter: RT-1 **discretizes** its actions. The 7 degrees of freedom of the arm — three for position, three for rotation, one for the gripper, plus a mode flag — each get chopped into **256 bins**. So "move a little left" becomes "output bin #131 on the x-axis," a discrete class the transformer predicts with a plain softmax. This is answer #1 from the last section, in its very first appearance: **actions become tokens, and control becomes classification.**
...
[FIG 9.2] title: "RT-1's action tokens"
type: slider-driven diagram
shows: a single continuous action value (a slider from -1 to +1, say the x-velocity of the gripper) mapped down onto a row of 256 discrete bins, with the selected bin highlighted; below, the reconstructed value the robot actually executes
controls: a slider for the true continuous action, and a "bin count" slider (4, 16, 64, 256) to change how finely the range is chopped
interaction: dragging the action slider lights up the bin it falls into and shows the quantized value the robot gets; lowering the bin count makes the staircase coarse and the reconstruction error visible
animation: the highlighted bin snaps as the slider crosses boundaries; a small red gap shows the difference between the true value and the binned value
color: blue = the true continuous action, amber = the chosen discrete bin, red = quantization error, grey = the unused bins
caption: "Binning turns control into classification: a continuous command snaps to the nearest of 256 bins. Simple and language-model-friendly — but the red gap is the fidelity you throw away, and it comes back to bite at high frequency."

---

## RT-2: give the body a brain (2023)
...
[FIG 9.3] title: "RT-2 speaks actions as text"
type: toggle-compare
shows: the same VLM backbone shown two ways — on the "text" tab it answers a visual question with words ("Q: what animal is extinct? A: dinosaur"); on the "action" tab, given a robot image and "pick up the extinct animal," it emits the integer action-string and the arm reaches for the toy dinosaur
controls: two tabs (Ask a question / Command the robot); a step button to reveal the token sequence one token at a time
interaction: toggling tabs shows that the *same model* produces either words or an action-string from the same vocabulary; stepping reveals that the action is literally a sequence of number-tokens sampled like any sentence
animation: tokens stream out one at a time; on the action tab the final string resolves into a highlighted arm motion toward the correct toy
color: blue = image features, amber = the emitted tokens (words or action integers), green = the correct grounded object (the dinosaur), grey = the model
caption: "RT-2's trick: an action is just a sentence of numbers. Because the backbone read the web, it can pick 'the extinct animal' with zero such examples in the robot data — semantics and motor control, finally in one model."

**Check your understanding.** RT-1 and RT-2 both discretize actions into bins and both control the same kind of arm. Why can RT-2 follow the instruction "move the banana to the number 3" while RT-1 cannot, even though neither ever saw that instruction in robot data?
...
## Open X-Embodiment: pool everyone's robots (2023)
...
[FIG 9.4] title: "Positive transfer across embodiments"
type: toggle-compare
shows: three robot embodiments (a Franka arm, a WidowX, a mobile manipulator) each with a small success-rate bar; a toggle switches between "trained solo on its own data" and "trained on the pooled OXE data," and the bars visibly rise under pooling
controls: a toggle "solo vs pooled," and tabs to focus on each embodiment
interaction: flipping to "pooled" raises every bar and lights arrows showing data flowing from the other robots into this one; a small callout notes RT-2-X's ~3× jump on emergent skills
animation: bars ease upward on the flip; transfer arrows fade in between the three robots
color: blue = the target robot, amber = data arriving from other embodiments, green = the success-rate gain, grey = baseline
caption: "Data from a different robot makes your robot better. Pooling 22 embodiments and a million trajectories produced positive transfer — the moment robotics got its own foundation-model story."

---

## OpenVLA: open it up, and make it small (2024)
...
[FIG 9.5] title: "OpenVLA's dual vision encoder"
type: toggle-compare
shows: one input image fed into two encoders side by side — SigLIP (labeled "semantics: what it is") producing a heat-map over the mug as an object, and DINOv2 (labeled "geometry: where its parts are") producing a heat-map over the handle and rim edges — then both feature streams merging into the shared policy
controls: three tabs (SigLIP only / DINOv2 only / both fused), plus a step button that runs the fused features into an action
interaction: viewing each encoder alone shows a characteristic blind spot — SigLIP knows it's a mug but is vague on the exact handle location; DINOv2 nails the geometry but doesn't "know" it's a mug — and the fused tab shows both strengths combined into a confident grasp point
animation: the two heat-maps fade in on their tabs; on "both," they overlay and a grasp marker snaps onto the handle
color: blue = geometric/spatial features (DINOv2), amber = semantic features (SigLIP), green = the resulting grasp point, grey = the raw image
caption: "Manipulation needs 'what' and 'where.' OpenVLA runs two encoders — SigLIP for semantics, DINOv2 for geometry — and beat a model 7× its size by seeing both. Open, small, and LoRA-tunable: the VLA anyone could build on."

**Check your understanding.** OpenVLA has 7B parameters and beat the 55B-parameter RT-2-X by 16.5%. What does this tell you about the relationship between model size and VLA performance, and what were the likely sources of the win?
...
## Octo: do you even need a pretrained VLM? (late 2023 → 2024)
...
[FIG 9.6] title: "Why diffusion actions handle ambiguity"
type: toggle-compare
shows: a cup with two equally-valid grasp approaches (left and right); on the "regression head" tab a single predicted action lands in the *middle* and misses the cup entirely; on the "diffusion head" tab the head represents both modes and commits to one valid grasp
controls: two tabs (single regression / diffusion), a "resample" button on the diffusion tab that re-runs the denoising and sometimes commits left, sometimes right
interaction: the regression tab always fails into the ambiguous middle; resampling the diffusion tab shows it choosing a *different valid* grasp each time — never the dead average
animation: on diffusion, a cloud of noise denoises over a few steps into a clean grasp vector that snaps to one side; on regression, the single output sits stubbornly in the middle
color: blue = the denoising action samples, amber = the committed action, green = a successful grasp, red = the failed average-of-two-modes
caption: "When two actions are equally good, averaging them is the worst answer. A diffusion head represents both modes and commits to one; a plain regression head smears into the useless middle — which is why continuous generative heads matter for manipulation."

---

## The action-representation problem, made explicit
...
[FIG 9.7] title: "Three ways to represent an action"
type: toggle-compare
shows: the same 7-DoF action chunk represented three ways in three tabs — (a) discrete bins, showing a long strip of ~50+ tokens and a coarse staircase reconstruction; (b) FAST tokens, showing a short strip of ~5-8 tokens and a smooth reconstruction; (c) continuous flow, showing no tokens but a small "generate" step and a smooth reconstruction — each tab displays a live token count and a fidelity score
controls: three tabs (binning / FAST / flow), and a "control frequency" slider that raises the action rate and visibly explodes the binning token count while FAST and flow stay compact
interaction: switching tabs updates the token count and fidelity readouts; cranking the frequency slider makes binning's token strip grow off the edge (turning red) while FAST barely moves — dramatizing exactly where binning breaks
animation: token strips grow/shrink on tab and slider changes; the reconstruction curve redraws with visible staircase (binning) vs smooth (FAST/flow)
color: blue = the true action trajectory, amber = the token representation, green = high fidelity, red = the runaway token count / low fidelity
caption: "The action-representation fork: binning is simple but explodes in length at high frequency; flow is high-fidelity but adds sampling cost; FAST compresses first so tokens survive. Every model after this fixes one corner of this triangle."

---

## FAST: compress in the frequency domain (Jan 2025)
...
1. **Quantile-normalize** each action dimension. Rescale each dimension by its data quantiles so they're on a comparable, outlier-robust footing.
2. **Discrete Cosine Transform (DCT)** each dimension. This is the workhorse — the same transform inside JPEG. The DCT re-expresses a time-series as a sum of cosine waves of increasing frequency, so instead of "the position at each of 50 timesteps" you now have "how much of each frequency is present." The slow, overall shape of the motion sits in the first few coefficients; the fast wiggle sits in the last ones.
3. **Scale, round, and drop** the insignificant high-frequency coefficients. After the DCT, most of the high-frequency coefficients are tiny — rounding them to zero throws away jitter you couldn't feel while keeping the motion's shape intact. This is the compression step; it's where the redundancy dies.
4. **Flatten low-frequency-first** into a single sequence, so the important coefficients lead.
5. **Byte-Pair Encoding (BPE)** the result — the same subword-merging tokenizer language models use — to squeeze it into a compact token string.
...
[FIG 9.8] title: "FAST tokenization: compress, drop, reconstruct"
type: stepper
shows: a wiggly action trajectory across four stages — (1) the raw trajectory over ~50 timesteps; (2) its DCT spectrum, a bar chart of coefficients with big low-frequency bars on the left and tiny high-frequency bars on the right; (3) the high-frequency bars greyed out / zeroed with a "dropped" label and the surviving few coefficients highlighted; (4) the reconstructed trajectory overlaid on the original, nearly identical, with a token-count readout ("52 raw → 6 tokens")
controls: play/pause to auto-advance the four stages, step forward/back, and a "drop threshold" slider that controls how many high-frequency coefficients get zeroed — showing the fidelity/compression trade live
interaction: raising the drop threshold zeros more coefficients (fewer tokens, more reconstruction error, shown as red divergence); lowering it keeps more (more tokens, tighter fit) — the reader feels the two-hyperparameter simplicity directly
animation: the DCT bars rise on entry; dropped bars fade to grey; the reconstruction curve redraws and eases onto the original
color: blue = the true trajectory, amber = the surviving low-frequency coefficients / tokens, grey = the dropped high-frequency coefficients, red = reconstruction error where the reader over-drops
caption: "FAST borrows JPEG's trick for actions: DCT into the frequency domain, drop the imperceptible high-frequency jitter, tokenize what's left. Ten-times fewer tokens, invertible, two knobs — and it let a discrete-token model match a diffusion model's quality while training up to 5× faster."

**Check your understanding.** FAST runs each action dimension through a Discrete Cosine Transform before tokenizing, then throws away most of the high-frequency coefficients. Why does this *dramatically* shorten the token sequence without noticeably hurting the robot's motion?
...
## π0: give it a flow-matching body (Oct 2024)
...
[FIG 9.9] title: "Flow matching an action chunk"
type: toggle-compare
shows: on the "flow" tab, a cloud of noise in action-space transported along near-straight arrows to a clean 50-step action chunk in just a few integration steps; on the "diffusion" tab, the same endpoint reached but along a curvy path taking many small steps — a step-count readout on each ("flow: 4 steps" vs "diffusion: ~50 steps")
controls: two tabs (flow / diffusion), a "steps" slider showing how few steps flow needs vs how many diffusion needs for the same quality, and a play button that animates the transport
interaction: watching flow reach the clean action chunk in a handful of near-straight moves while diffusion wanders there in many — the fidelity is equal but the step count (and thus latency) is not
animation: noise particles glide along the velocity field to form the action chunk; flow's paths are visibly straighter and shorter than diffusion's
color: blue = the noise / in-progress samples, amber = the emitted action chunk, green = the near-straight flow path, red/grey = the many-step diffusion path
caption: "π0's body is a flow-matching action expert: integrate a velocity field from noise to a 50-step action chunk in a few near-straight steps. Same continuous fidelity as diffusion, far fewer steps — a 3B VLM brain married to a fast, dexterous, chunked hand."

---

## π0.5: open-world and hierarchy (Apr 2025)
...
[FIG 9.10] title: "Hierarchical action: think, then move"
type: stepper
shows: a cluttered bedroom scene and the high-level goal "clean up"; the model steps through a sequence where it first emits a text subtask ("pick up the pillow"), then the flow-matching expert executes it, then it emits the next subtask ("put the book on the shelf"), and so on — a two-lane display with the text-plan lane on top and the motor-execution lane below
controls: step forward/back through subtasks, play/pause, and a toggle "hide the plan" that removes the text-subtask lane and shows performance dropping — dramatizing why the hierarchy helps
interaction: stepping advances one subtask at a time, showing the language plan generated first and the flow action executing it second; toggling off the plan lane degrades the success indicator
animation: each subtask's text appears, then the arm executes a short grasp/place; a small OOD-success meter reads 94% with the plan, 31% without multi-environment data
color: amber = the high-level language subtask (the plan), blue = the flow-matching motor execution, green = subtask completed, grey = the unstructured scene
caption: "π0.5 thinks in language, then acts in flow: emit a subtask ('pick up the pillow'), execute it with the flow expert, repeat — and it cleans homes it never trained in. Diverse co-training is the whole trick: 94% out-of-distribution with it, 31% without."

**Check your understanding.** π0.5's ablation shows 94% success on out-of-distribution tasks with multi-environment co-training and only 31% without it. Yet the action *architecture* (a flow-matching expert) is unchanged between the two. What does this tell you about where open-world generalization comes from in a VLA?
...
## ECoT: reason before you act (Jul 2024)
...
[FIG 9.11] title: "An embodied chain of thought"
type: stepper
shows: a tabletop scene and a goal; the reasoning ladder steps through TASK → PLAN → SUBTASK → MOVE → GRIPPER pixel (a marker drops onto the gripper in the image) → OBJECT bounding boxes (boxes snap onto the relevant objects) → ACTION (the arm moves), each step revealed in sequence with its text and, for the grounded steps, its overlay on the image
controls: step forward/back through the seven stages, plus an editable "thought" field on the SUBTASK/OBJECT steps — the reader can change "target: red block" to "target: blue block" and watch the grounding and final action update
interaction: stepping reveals each reasoning stage; editing a thought (e.g. swapping which object is the target) re-grounds the bounding boxes and redirects the final action, demonstrating human-correctable reasoning
animation: text stages type in; the gripper marker and bounding boxes snap onto the image at their stages; the final action arrow eases toward the (possibly re-chosen) target
color: amber = the language reasoning steps (plan/subtask), blue = the visually-grounded overlays (gripper pixel, bounding boxes), green = the final grounded action, red = a thought the reader has edited/corrected
caption: "Naive 'think out loud' isn't enough for a robot — reasoning has to touch pixels. ECoT escalates from goal to plan to exact gripper pixels and object boxes before acting: +28% over OpenVLA with no new data, and because the thoughts are legible you can edit them to correct the robot mid-task."

---

## MolmoAct: reason in 3D (Aug 2025)
...
1. **Depth-aware perception tokens.** A **VQVAE** (a vector-quantized autoencoder — it encodes an input into a compact set of discrete "perception tokens") produces tokens that capture the scene's **geometric structure via depth.** So before reasoning about the task, the model builds an explicit, depth-aware, 3D-ish representation of the scene — not just a flat image.
2. **Plan in image space as waypoints.** The model then lays out a **visual reasoning trace**: a sequence of **waypoints** in image space marking the path the end-effector should follow. This is a spatial plan you can literally see drawn on the image — the reasoning trace is geometric, not verbal.
3. **Decode actions conditioned on the waypoints.** Finally it emits the motor commands, conditioned on the depth tokens and the waypoint plan from the first two stages.
...
[FIG 9.12] title: "MolmoAct reasons in 3D, then plans a path"
type: stepper
shows: three stages — (1) an input image being encoded by a VQVAE into depth-aware perception tokens, visualized as a coarse depth map lighting up the scene's geometry; (2) a set of waypoints drawn onto the image tracing the end-effector's planned path around an obstacle; (3) the action decoder producing the motion that follows those waypoints
controls: step forward/back through the three stages, plus a "sketch correction" mode where the reader can drag a waypoint to a new position and watch the planned path — and the final action — update
interaction: stepping shows depth tokens → waypoint plan → action; in correction mode, dragging a waypoint re-routes the visible trajectory and the executed motion follows, demonstrating steerability
animation: the depth map fades in; waypoints drop onto the image in sequence and connect into a path; the arm traces the (possibly corrected) path
color: blue = the depth-aware perception, amber = the waypoint spatial plan, green = the executed 3D-grounded action, red = a waypoint the reader has dragged to correct the path
caption: "MolmoAct inserts 3D between seeing and acting: encode depth-aware perception tokens, plan a visible waypoint path, then decode the action along it. Because the plan is a drawn trajectory you can sketch over, the robot is steerable — reasoning grounded in geometry, not just pixels. (Benchmark numbers vendor-reported.)"

**Check your understanding.** ECoT and MolmoAct both "reason before acting," and both let a human correct the robot. What is the key difference in *what they ground their reasoning in*, and why does MolmoAct's choice help with manipulation specifically?
...
## Gemini Robotics & ER: reasoning as a plug-in brain (Mar 2025 →)
...
## GR00T N1: the dual-system humanoid (Mar 2025)
...
[FIG 9.13] title: "Dual-system control: slow brain, fast reflex"
type: animated pipeline
shows: two stacked lanes — the top lane is System 2 (the Eagle-2 VLM) ticking slowly at 10 Hz, emitting a plan; the bottom lane is System 1 (the flow-matching Diffusion Transformer) ticking fast at 120 Hz, consuming the latest plan and streaming motor commands to a humanoid that stays balanced; a rate meter shows the 12:1 tick ratio
controls: play/pause, a "System 2 rate" slider (drop it toward 1 Hz to show the plan going stale and the humanoid struggling) and a "System 1 rate" slider (drop it and watch the humanoid lose balance)
interaction: slowing System 1 makes the humanoid wobble and fall (too slow to balance); slowing System 2 makes it act on stale plans (semantically confused but still upright) — the reader feels *why* the two systems must run at different rates
animation: System 2 emits a plan token every 12 ticks of System 1; the fast lane streams continuous commands; the humanoid maintains a calm idle balance while executing
color: amber = the slow System-2 plan (the command), blue = the fast System-1 motor stream (the controlled signal), green = balanced/successful, red = falling or stale-plan failure, grey = the body structure
...
## STATE OF THE ROSTER (as of Aug 2026)
