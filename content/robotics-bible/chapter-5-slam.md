# Chapter 5 · Estimation, Localization & SLAM
...
## Belief is a distribution, not a point
...
[FIG 5.1] title: "Belief as a Gaussian: predict widens, update sharpens"
type: stepper
shows: a 1D Gaussian bell curve over a position axis; a "true" position marked in green; the bell's mean and ±σ band drawn explicitly
controls: play/pause, step, reset, and two buttons — "PREDICT (move)" and "UPDATE (measure)" — plus a readout of current μ and σ
interaction: pressing PREDICT slides the bell along the axis (motion) and visibly flattens/widens it (uncertainty grows); pressing UPDATE folds in a measurement and the bell grows taller and narrower (uncertainty shrinks); the two can be alternated
animation: the curve morphs smoothly between shapes over ~500ms, eased; the ±σ band expands on predict and contracts on update
color: blue = the belief curve, green = true position, amber = the incoming measurement, grey = position axis and gridlines
caption: "A belief is a bell curve: the mean is your best guess, the width is your doubt. Motion widens it, measurement sharpens it — that rhythm is the whole chapter."

**Check your understanding.** Two robots both report "I'm at x = 3.0 m." Robot A's belief has σ = 0.05 m; robot B's has σ = 0.8 m. They then get a measurement saying x = 3.4 m. Which robot should move its estimate more toward the measurement, and why?
...
## The Bayes filter: predict, then correct — forever
...
[FIG 5.2] title: "The Bayes filter loop"
type: animated pipeline
shows: two boxes in a cycle — PREDICT (motion model) and UPDATE (measurement model) — with the belief distribution drawn between them and its uncertainty ellipse redrawn at each stage
controls: play/pause, step, speed slider, and a toggle "measurements off" that lets predict run alone so the reader watches the belief balloon without correction
interaction: with measurements on, the ellipse pulses wider (predict) then tighter (update) each cycle; with measurements off, it grows without bound, dramatizing pure dead-reckoning drift
animation: a token flows predict→update→predict; the ellipse animates wider then narrower in sync
color: blue = the belief/ellipse, amber = motion command, green = measurement, red = the runaway growth when measurements are off
caption: "Two beats forever: predict widens the belief (motion adds doubt), update sharpens it (measurement removes doubt). Turn measurements off and watch dead-reckoning drift blow the ellipse up without limit."

**Check your understanding.** A robot runs its Bayes filter but its exteroceptive sensor (camera) fails, so only the PREDICT step keeps firing. Describe what happens to its belief over the next minute, and connect it to a villain from Chapter 1.
...
## The Kalman filter: the exact answer when the world is linear and Gaussian
...
$$K \ \approx\ \frac{\text{uncertainty in your prediction}}{\text{uncertainty in your prediction} + \text{uncertainty in your measurement}}$$
...
$$\sigma^2_{\text{fused}} \ =\ \frac{1}{\tfrac{1}{\sigma^2_A} + \tfrac{1}{\sigma^2_B}} \ =\ \frac{1}{\tfrac{1}{0.09} + \tfrac{1}{0.04}} \ =\ \frac{1}{11.11 + 25.0} \ =\ \frac{1}{36.11} \ \approx\ 0.0277.$$
...
$$\mu_{\text{fused}} \ =\ \sigma^2_{\text{fused}} \left(\frac{\mu_A}{\sigma^2_A} + \frac{\mu_B}{\sigma^2_B}\right) = 0.0277\left(\frac{3.0}{0.09} + \frac{3.6}{0.04}\right) = 0.0277\,(33.33 + 90.0) \approx 3.42\ \text{m}.$$
...
[FIG 5.3] title: "Kalman fusion: two noisy estimates make a tighter one"
type: slider-driven diagram
shows: three Gaussian curves on one position axis — estimate A (blue), estimate B (amber), and their fused result (green) — with each curve's σ and the fused σ printed as live readouts
controls: a slider for σ_A, a slider for σ_B, sliders for the two means μ_A and μ_B, and a reset
interaction: dragging either noise slider re-shapes that input curve and instantly recomputes the fused curve; the reader watches the green fused curve stay *taller and narrower than both* inputs no matter how the sliders move, and watches the fused mean slide toward whichever estimate is more confident
animation: all three curves morph smoothly as sliders move; the fused-σ readout updates live and flashes when it dips below both inputs
color: blue = estimate A, amber = estimate B, green = fused belief, grey = axis
caption: "Fusion beats its own ingredients: combine two noisy Gaussians and the result is sharper than either one, pulled toward whichever you trust more. The Kalman gain is that trust, computed from the variances."

**Check your understanding.** In the fusion above, suppose estimate B (the landmark) suddenly becomes very uncertain — its σ_B balloons to 2.0 m. Without recomputing exactly, what happens to the fused mean and to the Kalman gain, and why?
...
## The Extended Kalman Filter: linearize, because real robots bend
...
**Check your understanding.** Why does a *large* uncertainty (a fat covariance ellipse) make the EKF's linearization especially dangerous, whereas a *small* one is usually fine?
...
## Particle filters: when belief can't be a single blob
...
[FIG 5.4] title: "Monte Carlo Localization: a particle cloud finding itself"
type: stepper
shows: a top-down map with a few landmarks/doors and a hidden true robot pose (green); the belief drawn as a scatter of particle dots; a step counter k/N
controls: play/pause, step, reset, a "kidnap" button that re-scatters particles uniformly, and a slider for particle count
interaction: each step runs predict→reweight→resample; the reader watches particles (initially spread over the whole map) form clusters at ambiguous spots, then collapse to a single tight cluster on the true pose as more measurements arrive; pressing "kidnap" re-spreads them to show recovery
animation: particles drift and jitter on predict, brighten/dim by weight on update, and snap into clusters on resample — smooth, eased, seeded so it's identical every load
color: blue = particles (brightness = weight), green = true pose, amber = landmarks/measurements, grey = the map
caption: "A cloud of guesses can believe what a bell curve can't: 'I'm at one of these three doors.' Measurements reweight the particles, resampling concentrates them, and the cloud collapses onto the truth."

**Check your understanding.** In the kidnapped-robot scenario, why does the particle filter recover gracefully while an EKF would likely fail outright?
...
## Maps: something to localize against
...
[FIG 5.5] title: "An occupancy grid filling in as the robot scans"
type: stepper
shows: a top-down room with true walls (faint), a robot that drives a short path, and a grid of cells shaded by occupancy probability (dark = free, bright = occupied, grey = unknown 0.5)
controls: play/pause, step, reset, and a slider for sensor noise that makes the emerging map crisper or fuzzier
interaction: each step the robot moves and fires a fan of LiDAR rays; cells the rays cross darken toward "free," cells where they stop brighten toward "wall"; raising the noise slider shows walls emerging blurrier and needing more sweeps to sharpen
animation: rays sweep out each step; cell shades update smoothly; the room's walls emerge from grey over successive steps
color: grey = unknown (0.5), dark = free space, bright/amber = occupied, blue = the robot and its rays, green = true wall outline for reference
caption: "A map is a belief too: each cell holds the probability it's occupied, updated by every ray that crosses or stops in it. No single scan is trusted — the walls emerge from consensus."
...
**Check your understanding.** A SLAM robot drives down a 50 m corridor, and each 1 m step it takes adds a tiny random heading error averaging just 0.2°. Intuitively, why does the *far end* of the corridor end up meters off in the map even though every individual step looked nearly perfect?
...
## Loop closure: the single best trick in SLAM
...
[FIG 5.6] title: "Loop closure snaps a drifted trajectory back into place"
type: stepper
shows: a robot driving a large loop through a building; its *estimated* trajectory (blue) visibly drifts away from the *true* trajectory (green) as it goes, and the built map skews with it; pose nodes and odometry edges drawn along the path
controls: play/pause, step, reset, and a prominent "close the loop" button that becomes active once the robot returns near its start
interaction: the reader steps the robot around the loop watching drift accumulate (blue peeling away from green); when it revisits the start, pressing "close the loop" adds a loop-closure edge and the whole trajectory + map snaps into alignment with the truth in one animated correction
animation: drift grows smoothly along the loop; on loop closure, the entire pose graph relaxes in one eased motion (~700ms) as error redistributes around the loop and blue collapses onto green
color: blue = estimated trajectory/map (drifted), green = true trajectory, amber = odometry edges, red = the accumulated drift gap, bright highlight = the loop-closure edge
caption: "Recognizing a place you've been before is the master key: one loop-closure constraint pins the drifted loop at both ends, and the optimizer redistributes an hour of accumulated error in a single snap."

**Check your understanding.** A SLAM front-end mistakenly decides two different-but-similar hallways are the same place and adds a loop-closure edge between them. What happens when the back-end optimizes, and why is this failure so much worse than simply missing a real loop closure?
...
## Flavors: visual SLAM, LiDAR SLAM, and where it's heading
...
**Check your understanding.** Why does loop closure — the trick that saves SLAM from drift — tend to be *easier* for visual SLAM than for LiDAR SLAM, and what does that tell you about why real systems fuse both?
