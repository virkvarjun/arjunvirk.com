  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        `Here's the setup, and it's pure supervised learning. A demonstration is a stream of paired snapshots: at each instant you record what the robot **observed** (call it $o$: camera images, maybe joint angles) and what the expert **did** (call it $a$: the action, usually the next target joint positions or end-effector pose). Slice the whole demonstration into these $(o, a)$ pairs and you have a labeled dataset, exactly like a pile of (image, label) pairs for a classifier. Then you train a network $\\pi$, the **policy**, to predict the action from the observation:`,
      ],
    },
    {
      equations: [
        `\\min_{\\pi}\\; \\sum_{(o,\\,a)\\,\\in\\,\\mathcal{D}} \\bigl\\lVert\\, \\pi(o) - a \\,\\bigr\\rVert^{2}`,
      ],
    },
    {
      paragraphs: [
        `Reading the symbols: $\\pi(o)$ is the action the policy *guesses* when it sees observation $o$; $a$ is the action the expert *actually took* in that situation; $\\mathcal{D}$ is your pile of demonstration pairs; the $\\lVert \\cdot \\rVert^2$ is squared error, the gap between guess and truth. You nudge the network's weights to shrink that gap across the whole dataset. When you're done, you have a function that maps "what I see" straight to "what to do," and you run it in a loop: observe, feed it through $\\pi$, execute the action, observe again. Sense → plan → act, where "plan" is one forward pass through a trained net.`,
    ...
    },
    {
      diagram: {
        id: "rb-10-1",
        caption:
          "Behavior cloning is nothing exotic: slice expert demos into (observation, action) pairs and fit a function that maps what you see to what to do — and the deployed policy is only reliable where those pairs actually cover the states it visits. Simple, no reward needed, and the entire backbone of modern VLAs.",
      },
    },
    {
      quiz: {
        ...
        answer:
          "Because a classifier's mistakes are independent (get one image wrong, the next image is unaffected) but a policy's mistakes *change its own future inputs*. The action the policy takes right now determines what it observes next, which determines its next action, and so on down the trajectory. That coupling (action changing future observation, the exact thing Chapter 1 said makes robots hard) is invisible in the training setup, since you fit on static pairs, but it dominates at deployment. A supervised classifier trained and tested on the same distribution is fine; a BC policy trained on the expert's states but *running on its own states* is a fundamentally different, harder situation. That gap is the entire subject of the next section.",
      },
    },
    {
      ...
      paragraphs: [
        ...
        `Let's put a number on how bad this is, because the scaling is genuinely alarming. Ross and Bagnell showed (this is the analysis behind DAgger, 2011) that if a BC policy makes a mistake with some small probability $\\varepsilon$ on each step, the *total* expected error over a trajectory of length $T$ doesn't grow like $T\\varepsilon$ the way you'd hope; it grows like $T^2\\varepsilon$. Quadratic in the horizon. Read that again: doubling the length of the task quadruples the accumulated error. Plug in numbers to feel it: with $\\varepsilon = 0.01$ and $T = 100$, the hoped-for $T\\varepsilon$ says "about one bad step per run"; the real bound, $T^2\\varepsilon$, allows a hundred units of accumulated cost, because each bad step degrades every step after it. A policy with a 1% per-step error rate over a 100-step task doesn't have a 1% problem; the errors have room to feed on themselves across all 100 steps, and the trajectory-level failure rate blows up. This is why a policy can ace short reaches and fall apart on anything long and multi-stage. The horizon is the enemy.`,
    ...
    },
    {
      diagram: {
        id: "rb-10-2",
        caption:
          "This is imitation's core flaw, simulated honestly: every step taken from a state outside the training tube degrades the next one, so the measured error curve bends upward like $T^2$, not $T$. DAgger fights back by having the expert label the states the policy actually visits, and the same measured curve flattens toward linear.",
      },
    },
    {
      quiz: {
    ...
    },
    {
      diagram: {
        id: "rb-10-3",
        caption:
          "The same noisy simulation as Fig 10.2, run twice with the same seed: predicting a chunk of actions at once slashes the number of closed-loop seams where fresh error can enter, and the measured drift curve drops accordingly. This is ACT's trade: longer chunks buy stability at the price of reacting only at the seams.",
      },
    },
    {
      quiz: {
        question:
          "**Check your understanding.** Chunking reduces compounding error partly by re-planning less often. But a one-step policy also has a specific failure, where it gets \"stuck\" and stops moving, that isn't obviously about error accumulation. Why does predicting chunks fix the sticking problem too?",
        answer:
    ...
    },
    {
      ...
      paragraphs: [
        ...
        `Think about walking around a pole to reach a door. Going left is fine. Going right is fine. Both are things a sensible demonstrator does, and across a dataset you'll see plenty of both. Now ask a plain squared-error policy (the BC loss from the first section) to predict "the action" here. What does it do? It minimizes average squared error, and the action that minimizes average squared error between "go left" and "go right" is... **go straight into the pole.** Put numbers on it: if half the demos at this state say $a = +1$ (veer left) and half say $a = -1$ (veer right), the squared-error minimizer is their mean, $a = 0$ — dead center. The average of two good answers is a terrible answer. This is the **multimodality** problem: when a task has several distinct valid solutions, a regressor that predicts one action smears them into the mean, and the mean is often exactly the one thing that fails. Every "reach for the object from the left or from the right," "grasp the pan by the handle or the rim," "stack it here or there" hides this trap.`,
    ...
    },
    {
      diagram: {
        id: "rb-10-4",
        caption:
          "When a task has several right answers, squared-error regression predicts their average — computed here as the actual mean of the demo set, and it drives straight into the obstacle. Diffusion Policy instead denoises seeded noise toward the demo distribution: the posterior mean starts as that same collapsed average, then sharpens until the sample commits cleanly to left or right, and resampling flips the lane.",
      },
    },
    {
      quiz: {
    ...
    },
    {
      diagram: {
        id: "rb-10-5",
        caption:
          "ALOHA: the operator backdrives a cheap leader arm and the larger follower mirrors its joint angles, so the recorded demonstration — the green end-effector trace — lands exactly in the follower robot's action space: high-quality, embodiment-matched data, one slow stream at a time.",
      },
    },
    {
      diagram: {
        id: "rb-10-6",
        caption:
          "UMI decouples data collection from the robot: a human traces the demonstration anywhere with a handheld gripper and GoPro, and because the jaws and camera viewpoint are built to match a real robot's, the robot can replay the very same recorded path. Throughput and diversity soar; the price is matching the interface.",
      },
    },
    {
      quiz: {
