...
export const chapter4: Chapter = {
  ...
  sections: [
    ...
    {
      list: [
        "Error: $e = 90 - 60 = 30^\\circ$. Command: $u = 2 \\times 30 = 60$ N·m. Big error, big push: the arm accelerates toward the target.",
        "It swings to 85°. Now $e = 90 - 85 = 5^\\circ$, so $u = 2 \\times 5 = 10$ N·m. Closer, so the controller eases off. Good: this is the self-throttling behavior that makes P feel so reasonable.",
    ...
    },
    {
      paragraphs: [
        ...
        "The arm can only produce effort when there's error, because $u = K_p e$. So to generate the 8 N·m needed just to hold position, it *needs* a standing error big enough that $K_p e = 8$. With $K_p = 2$, that's $e = 8/2 = 4^\\circ$. The arm settles not at 90° but at **86°**, permanently four degrees short, and it parks there forever, perfectly balanced: the 4° of error produces exactly the 8 N·m that gravity is stealing. Push the gain up to $K_p = 8$ and the standing error shrinks to $8/8 = 1^\\circ$, better but still not zero. Crank it higher and you buy a smaller error at the price of a twitchy, oscillation-prone controller that rings on every move.",
    ...
    {
      paragraphs: [
        "Reading the symbols: **$\\int e\\,dt$** is the **integral of the error**, the running sum of all past error, area under the error-versus-time curve. If you've been 4° short for a while, this term is large and growing; if the error has been zero, it stops growing and holds whatever value it reached. **$K_i$** is the **integral gain**, setting how fast the accumulator translates into effort. In code you never integrate symbolically. You just keep a variable and, every timestep of length $\\Delta t$, do `integral += e × Δt`. That accumulator *is* the memory P was missing.",
        "By hand, with $K_i = 0.5$ and a timestep $\\Delta t = 0.1$ s, sitting at the P-controller's stuck 4° error:",
      ],
    },
    {
      list: [
        "Step 1: $\\text{integral} = 0 + 4 \\times 0.1 = 0.4$. Integral effort: $0.5 \\times 0.4 = 0.2$ N·m. A small extra shove upward, on top of whatever P is doing.",
        "Step 2: still 4° short, so $\\text{integral} = 0.4 + 4 \\times 0.1 = 0.8$. Effort: $0.5 \\times 0.8 = 0.4$ N·m. The push grows.",
    ...
    {
      list: [
        "**$K_p \\cdot e$, the present.** *How wrong am I right now?* Push in proportion to the current error. This does the bulk of the work.",
        "**$K_i \\cdot \\int e\\,dt$, the past.** *How long have I been wrong?* Accumulate persistent error and grind out the steady-state gap that P leaves behind.",
        "**$K_d \\cdot de/dt$, the future.** *How fast am I closing?* Anticipate arrival and brake early to kill overshoot.",
      ],
    },
    {
      paragraphs: [
        "Present, past, future: one signal, three lenses, three gains to blend them. That's the whole thing. And it's genuinely ancient by software standards: the theory was worked out by **Nicolas Minorsky in 1922** for automatic ship steering. He derived the three-term law by watching how a skilled helmsman actually held a course: hard over for a big heading error (P), a little extra to counter a steady crosswind (I), and easing the wheel back as the bow swung toward the mark (D). A century later, PID still runs an estimated 90-plus percent of the industrial control loops on the planet: thermostats, cruise control, drone attitude, 3D-printer heaters, hard-drive heads. It is the workhorse, and understanding it is most of practical control.",
    ...
    {
      paragraphs: [
        "Reading it: **$u_{\\text{ff}}$** is the **feedforward term**, computed from a model of the plant; for an arm, most commonly the **gravity-compensation torque**, the exact torque needed to hold the arm up against gravity at its current pose. The feedback block is our familiar PID, now demoted to a *residual cleaner*: it only has to correct the difference between what the model predicted and what actually happened (friction, an unmodeled load, a gust). Prediction does the heavy lifting; feedback mops up.",
        "The canonical case: **gravity compensation.** For a single joint holding a link of mass $m$ whose center of mass sits a distance $L$ from the pivot, the gravitational torque trying to pull it down is $\\tau_g = m g L \\cos\\theta$, where $g = 9.81$ m/s² and $\\theta$ is measured from horizontal. That $\\cos\\theta$ is the geometry: gravity's torque is maximal when the arm is horizontal ($\\cos 0^\\circ = 1$, full lever arm) and zero when it hangs straight down ($\\cos 90^\\circ = 0$, no lever arm). Work a number: $m = 2$ kg, $L = 0.3$ m, arm horizontal ($\\theta = 0$). Then $\\tau_g = 2 \\times 9.81 \\times 0.3 \\times 1 = 5.9$ N·m. Feed that 5.9 N·m in as $u_{\\text{ff}}$ and the arm holds itself level *with zero error* before feedback has done anything at all: you've cancelled gravity outright instead of tolerating a standing error to fight it.",
    ...
    },
    {
      ...
      paragraphs: [
        ...
        "The fix is small and elegant: **make the setpoint a moving target.** Instead of a fixed $r$, feed the controller $r(t)$, the desired position *right now*, and recompute the error against it every timestep: $e(t) = r(t) - y(t)$. The controller is now chasing a point that's itself gliding along the path, and its job is to stay glued to it. This pairs perfectly with feedforward: since you *planned* the trajectory, you know the desired *velocity* and *acceleration* at every instant, so you can feed those forward, commanding the motor the acceleration the plan calls for (mass times desired acceleration, plus the gravity term), and let PID clean up only the tracking error. Feedforward the plan, feed back the deviation.",
        "Put a number on it. Plan a sweep from 0° to 90° over 3 s with a smooth cosine ramp, $r(t) = 45(1 - \\cos(\\pi t / 3))$ degrees. At $t = 1.5$ s the plan says: be at 45°, moving at 47 °/s, accelerating at exactly zero (it's mid-swing, coasting through the fastest point). Feedforward hands the motor the torque for precisely that state, the gravity term at 45° plus inertia times the planned acceleration, and PID's whole job shrinks to trimming the last degree of tracking error. Chase the moving point, not the far-off endpoint, and the error never gets a chance to grow. Trajectory tracking is where feedforward really earns its keep, because now you're anticipating not just gravity but the whole intended motion. (Where those smooth $r(t)$ trajectories *come from*, planning a feasible, collision-free path through the configuration space of Chapter 1's Fig 1.3, is the subject of Chapter 6. Here we just track one once it exists.)",
      ],
    },
    {
    ...
    {
      paragraphs: [
        "Reading the symbols: **$F$** is the force the robot exerts. **$x$** is the actual end-effector position, **$x_{\\text{desired}}$** the target (the rest position of your virtual spring). **$K$** is the **stiffness** (the virtual spring constant you get to *choose*) and **$D$** is the **damping**. If you notice this looks exactly like a spring-damper from physics class, that's the entire point: the robot is *emulating* a spring of stiffness $K$ and a damper $D$, in software, at whatever values you dial in. Push the end-effector away from $x_{\\text{desired}}$ and it pushes back with force $K$ times the displacement, gently if $K$ is small, firmly if $K$ is large, exactly like a real spring, except *you* set the spring constant on the fly.",
        ...
        "Work the number: $K = 200$ N/m, virtual rest position $x_{\\text{desired}}$, and something pushes the end-effector 2 cm ($0.02$ m) off it. Restoring force: $F = 200 \\times 0.02 = 4$ N. A gentle, springy resistance: you can push it around by hand and it yields like a soft spring. Now set $K = 5000$ N/m and the same 2 cm displacement demands $F = 5000 \\times 0.02 = 100$ N: rigid, immovable, a position controller in all but name. Same robot, same code, one number deciding whether it's a rock or a cushion.",
