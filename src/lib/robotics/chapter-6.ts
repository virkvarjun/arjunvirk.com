...
export const chapter6: Chapter = {
  ...
  sections: [
    ...
    {
      list: [
        "For a **mobile base** on the floor, a configuration is $(x, y, \\theta)$: where it is and which way it faces. C-space is 3-dimensional. For a base that's just a dot rolling around and you ignore heading, it's the plain 2D floor.",
        "For a **robot arm**, a configuration is the tuple of joint angles: $(\\theta_1, \\theta_2, \\dots, \\theta_n)$ for an $n$-joint arm. **C-space is joint space**, one axis per joint. A 6-joint arm has a 6-dimensional C-space, and a point in it is one complete arm posture. This is exactly Fig 1.3, just with more axes.",
      ],
    },
    {
      paragraphs: [
    ...
    },
    {
      ...
      paragraphs: [
        "The first crack in BFS is that \"shortest in steps\" isn't \"shortest in cost.\" Real terrain has costs: diagonal moves are longer than straight ones ($\\sqrt{2}$ vs 1), mud is slower than road, going near a wall is riskier. **Dijkstra's algorithm (1959)** fixes this. Instead of expanding in step-count rings, it keeps a running tally $g(n)$ (the cheapest known cost to reach each cell from the start) and always expands the *unexpanded cell with the smallest g so far*. When it finally pops the goal off that priority queue, $g(\\text{goal})$ is provably the true cheapest cost, and you recover the path by following the \"who got me here cheapest\" pointers backward.",
    ...
    {
      paragraphs: [
        "Reading the symbols: $g(n)$ is the cost already spent getting *to* cell $n$ from the start (the part you know for sure, same as Dijkstra). $h(n)$ is the *estimated* cost still to go from $n$ to the goal (the guess). Their sum $f(n)$ is $\\mathrm{A}^*$'s best estimate of the total cost of a path through $n$. By always expanding the smallest-$f$ cell, the search leans toward cells that are both cheap to reach *and* look close to the goal, so it drives *at* the goal instead of ballooning in every direction.",
        "What do you use for $h$? Something cheap that estimates remaining distance. On a grid where you can move in 8 directions, straight-line (Euclidean) distance is a classic choice; if you can only move in 4, the Manhattan distance ($|\\Delta x| + |\\Delta y|$) fits. The one property that matters is:",
        "**Admissibility: the heuristic must never overestimate the true remaining cost.** As long as $h(n)$ never exceeds the real cost-to-go for any cell, $\\mathrm{A}^*$ is guaranteed to return the optimal path. The intuition: an admissible $h$ is optimistic; it might undersell how far the goal is, but it never oversells. Because it never claims the goal is farther than it truly is, $\\mathrm{A}^*$ can never be tricked into finalizing a suboptimal path and skipping a better one. Straight-line distance is always admissible in the plane, because no path through obstacles is ever *shorter* than the straight line.",
        "And now the punchline that ties the three together:",
      ],
    },
    {
      list: [
        "Set $h(n) = 0$ everywhere and $\\mathrm{A}^*$ stops guessing: it's exactly **Dijkstra**.",
        "Add positive terrain costs on top of BFS's uniform steps and you *get* **Dijkstra**.",
        "So $\\mathrm{A}^*$ **is Dijkstra plus an admissible heuristic that pulls the search toward the goal.** Same optimality guarantee, a fraction of the expansions.",
      ],
    },
    {
      heading: "A tiny trace",
      paragraphs: [
        "Let's watch $\\mathrm{A}^*$ actually run. Tiny grid, 4-connected (up/down/left/right, each step costs 1). Start `S` at column 0, goal `G` at column 3, same row. One obstacle `#` blocks the direct middle. Coordinates are (col, row), and the heuristic is Manhattan distance to G at (3,1).",
      ],
    },
    {
      code: {
...
row 2:  .   .   .   .`,
    ...
    {
      list: [
        "**Expand S**=(0,1): $g=0$. Neighbors: (0,0), (0,2), and (1,1). (1,1) is the obstacle, skip. (0,0): $g=1$, $h=|3-0|+|1-0|=4$, $\\mathbf{f=5}$. (0,2): $g=1$, $h=4$, $\\mathbf{f=5}$.",
        "Tie at $f=5$; say we take **(0,0)**: $g=1$. Its useful neighbor is (1,0): $g=2$, $h=|3-1|+|1-0|=3$, $\\mathbf{f=5}$.",
        "**Expand (1,0)**: $g=2$. Neighbor (2,0): $g=3$, $h=|3-2|+|1-0|=2$, $\\mathbf{f=5}$. (Notice: every cell on the good detour keeps $f=5$, and that's the heuristic telling $\\mathrm{A}^*$ it's still on an optimal path.)",
        "**Expand (2,0)**: $g=3$. Neighbor (2,1): $g=4$, $h=|3-2|+0=1$, $\\mathbf{f=5}$.",
        "**Expand (2,1)**: $g=4$. Neighbor **(3,1)=G**: $g=5$, $h=0$, $\\mathbf{f=5}$. Goal reached.",
      ],
    },
    {
      paragraphs: [
        "Path: S → (0,0) → (1,0) → (2,0) → (2,1) → G, cost 5, going up and over the obstacle. Here's the thing to notice: $\\mathrm{A}^*$ barely touched the cells *below* the start row. Dijkstra, with $h=0$, would have expanded (0,2), (1,2), (2,2), the whole bottom row, with equal enthusiasm, because it can't tell the goal is up-and-right. The heuristic kept $\\mathrm{A}^*$'s $f$-values honest and steered it over the top, expanding a handful of cells instead of flooding the grid. That's the entire reason $\\mathrm{A}^*$ is the default planner in games, GPS routing, and a thousand robots.",
      ],
    },
    {
      diagram: {
        id: "rb-6-2",
        caption:
          "Same optimal path, wildly different effort. Dijkstra expands a blind blob in every direction; $\\mathrm{A}^*$'s heuristic aims the search at the goal, so it finalizes the same path after a fraction of the expansions.",
      },
    },
    {
      quiz: {
        question:
          "A colleague sets $\\mathrm{A}^*$'s heuristic to $h(n) = 3 \\times$ (straight-line distance to goal), reasoning that a stronger pull will reach the goal even faster. The search does run faster, but sometimes returns a slightly longer path. What broke?",
        answer:
          "They broke admissibility. Multiplying the straight-line distance by 3 makes $h$ *overestimate* the true cost-to-go for most cells; it now claims the goal is much farther than it really is. That exaggeration makes the search greedy: $h$ dominates $f$, so the search rushes toward whatever looks closest and can commit to a path before discovering a cheaper one it dismissed as \"too far.\" So yes, it expands fewer cells and finishes faster, but it loses $\\mathrm{A}^*$'s optimality guarantee and can return a suboptimal path. This is a real and useful knob (**weighted** $\\mathrm{A}^*$ deliberately inflates the heuristic to trade a bounded amount of optimality for speed) but it is no longer guaranteed optimal. Optimality lives and dies on $h(n)$ never overestimating.",
      },
    },
    {
      heading: "The curse of dimensionality: why the grid dies",
      paragraphs: [
        "Grid search is wonderful and you should reach for it first: on a mobile robot navigating a 2D floor, $\\mathrm{A}^*$ on an occupancy grid is often the entire answer. But now point it at a 6-joint arm, and watch it detonate.",
        "The problem is counting. Suppose you grid each axis into just 100 cells: coarse, barely enough to squeeze through a doorway. In 2D (a mobile base ignoring heading) that's $100 \\times 100 = 10^4$ cells. Totally fine; your laptop laughs. In 3D (add heading) it's $10^6$. Still fine. Now the 6-joint arm: $100^6 = 10^{12}$ cells. **A trillion.** Even if you could store one bit per cell that's 125 gigabytes just to hold the grid, and $\\mathrm{A}^*$ would have to expand a meaningful fraction of it. A 7-DoF arm is $10^{14}$. A two-armed humanoid with a floating base is astronomically worse.",
    ...
    },
    {
      quiz: {
        question:
          "Gridding a 2-DoF planar arm at 100 cells per joint gives $10^4$ cells, instant. Why does the *same* 100-cell resolution become hopeless for a 6-DoF arm, and why doesn't buying a faster computer fix it?",
        answer:
          "Because the cell count is 100 raised to the number of joints, so going from 2 to 6 DoF takes you from $100^2 = 10^4$ to $100^6 = 10^{12}$: a factor of a hundred million, because every extra joint multiplies the total by 100. It's *exponential* in dimension, and exponentials outrun hardware: a computer a thousand times faster only buys you a couple more joints before you're back in the same hole, and each added joint erases that gain. You cannot out-engineer an exponential with linear speedups. The escape isn't a bigger grid or a faster machine: it's refusing to enumerate the whole space at all, and instead sampling only the configurations you might actually use. That realization is the entire motivation for sampling-based planning.",
      },
    },
    {
    ...
    },
    {
      list: [
        ...
        "**Query.** Given a start and a goal, connect each to the nearest roadmap node, then run plain old $\\mathrm{A}^*$ on the roadmap graph to find a path. Done.",
      ],
    },
    {
      paragraphs: [
    ...
    {
      list: [
        "**Sample** a random configuration $q_{\\text{rand}}$ in C-space.",
        "**Find** the node already in the tree that's nearest to $q_{\\text{rand}}$, and call it $q_{\\text{near}}$.",
        "**Steer** a small step from $q_{\\text{near}}$ toward $q_{\\text{rand}}$, landing at a new point $q_{\\text{new}}$.",
        "**Check** the little segment $q_{\\text{near}} \\to q_{\\text{new}}$ for collision. If it's clear, add $q_{\\text{new}}$ to the tree with $q_{\\text{near}}$ as its parent.",
        "Repeat until a $q_{\\text{new}}$ lands close enough to the goal.",
      ],
    },
    {
      paragraphs: [
        "Why does this explode outward so efficiently? The magic is step 2. Because you always extend from the *nearest* existing node toward a *random* target, the tree is biased toward the large unexplored voids: a random sample is most likely to fall in a big empty region, and the nearest node to it is on the frontier facing that region. So the tree reaches into open space fast, \"rapidly exploring,\" instead of thrashing around where it already is. (Common trick: every so often, make $q_{\\text{rand}}$ *be* the goal instead of a random point. That \"goal bias\" tugs the tree toward the target without giving up the exploration.)",
        "RRT's real superpower, and the reason it dominates in practice, is that **the \"steer\" step can respect the robot's motion constraints.** Remember Chapter 1's nonholonomic car: it can't slide sideways, only drive-and-steer along feasible curves. In a grid or PRM you'd connect nodes with straight lines the car can't follow. But RRT's steer step can be *the car's actual dynamics*: extend $q_{\\text{near}}$ toward $q_{\\text{rand}}$ by simulating a legal drive-and-steer motion. Every edge in the tree is then a motion the robot can genuinely execute. This is why RRT and its kin are the go-to for nonholonomic and **kinodynamic** planning (planning that respects velocity and acceleration limits, not just geometry) from self-parking cars to quadrotors.",
      ],
    },
    {
    ...
    },
    {
      ...
      paragraphs: [
        "The fix is $\\mathbf{RRT}^*$ (Karaman & Frazzoli, 2011) and it's a small, gorgeous addition that makes the tree *self-improve.* $\\mathrm{RRT}^*$ runs the same sample-nearest-steer loop, but adds two rewiring steps every time it inserts a new node $q_{\\text{new}}$:",
      ],
    },
    {
      list: [
        "**Choose the best parent.** Instead of automatically parenting $q_{\\text{new}}$ to the nearest node, look at all nearby tree nodes and pick the one that gives $q_{\\text{new}}$ the *lowest total cost from the start* (via a collision-free connection). Connect through whichever neighbor makes the path to $q_{\\text{new}}$ cheapest.",
        "**Rewire the neighborhood.** Then check the other nearby nodes: for each, would routing *through the new node* give it a cheaper path than its current parent does? If so, re-parent it to $q_{\\text{new}}$. The tree locally reshuffles its connections to shorten paths.",
      ],
    },
    {
      paragraphs: [
        "That rewiring is everything. Over iterations, kinks straighten, detours collapse, and the tree's paths converge toward the true optimum. This is what \"**asymptotically optimal**\" means: $\\mathrm{RRT}^*$ isn't optimal after a fixed number of samples, but as samples $\\to \\infty$ the cost of its path converges to the best possible. In practice you run it for a fixed time budget and take the best path so far, and it's dramatically smoother and shorter than vanilla RRT's first jagged attempt. The trade is compute: all that neighbor-checking and rewiring costs more per sample. Vanilla RRT when you just need *a* path fast; $\\mathrm{RRT}^*$ when path quality is worth the cycles.",
      ],
    },
    {
      diagram: {
    ...
    },
    {
      diagram: {
        id: "rb-6-4",
        caption:
          "RRT takes the first path it finds and stops, so it stays jagged; $\\mathrm{RRT}^*$ keeps rewiring the tree to reroute nodes through cheaper parents, so its path smooths and its length converges toward optimal as iterations grow.",
      },
    },
    {
      quiz: {
    ...
    },
    {
      list: [
        ...
        "**Smoothness.** RRT's output has corners, and $\\mathrm{RRT}^*$'s is better but still not silky. A robot cannot infinitely-sharply change direction: a corner demands infinite acceleration, which means infinite force, which no motor has. Corners must be rounded into curves the body can actually flow through.",
    ...
    {
      paragraphs: [
        "The canonical local planner is the **Dynamic Window Approach (DWA)** (Fox, Burgard & Thrun, 1997). Its idea is direct and it leans hard on Chapter 1's holonomy lesson. At each tick, DWA looks only at the velocities the robot can *actually reach in the next instant* given its current speed and its acceleration limits; that reachable set is the \"dynamic window,\" and it exists precisely because the robot can't teleport to any velocity; it's bounded by its own dynamics. (Concretely: rolling at 0.5 m/s with a 1 m/s² acceleration limit and a 0.1 s control tick, the window holds only speeds between 0.4 and 0.6 m/s — a razor-thin slice of everything the motor could ever do, which is exactly why scoring every candidate in it is cheap.) For each candidate velocity in that window, DWA simulates the short arc the robot would trace and scores it on three things: does it head toward the global goal, does it stay clear of obstacles, and does it keep speed up. Pick the best-scoring velocity, execute it for one tick, and repeat. It's sense-plan-act at the reflex level: a tight, fast loop that hugs the global plan but flinches away from anything new.",
    ...
    },
    {
      quiz: {
        question:
          "Why not just run the global planner ($\\mathrm{A}^*$ on the full map) at 30 Hz and skip the local planner entirely; wouldn't that both find the optimal route *and* react to obstacles instantly?",
        answer:
          "Two reasons, one practical and one conceptual. Practically, global planning over a full map is too slow to run at reflex rates: a fresh $\\mathrm{A}^*$ or sampling search across the whole environment can take tens or hundreds of milliseconds, and by the time it finishes the fast-moving obstacle has moved and the plan is already stale. You can't close a 30 Hz reactive loop around a planner that takes longer than a tick. Conceptually, the two layers reason about different things: the global planner needs the *whole map* to find a good route but doesn't need to know about the person who just appeared; the local planner needs *only the immediate surroundings and the robot's dynamics* to dodge, and doesn't need to re-solve the whole maze to do it. Splitting them lets each run at the rate its job actually needs, slow-and-global for strategy, fast-and-local for reflexes, which is the same \"fuse things with complementary strengths\" pattern we've used since Chapter 1's sensors.",
      },
    },
    {
      heading: "Where this leaves us",
      paragraphs: [
        "The robot can now go somewhere. Chapter 5 gave it a pose on a map; this chapter turned \"I'm here, I want to be there\" into an executable motion. The spine of it: **planning is drawing a path through configuration space**, the space where the robot shrinks to a point (Fig 1.3's promise, cashed in via obstacle inflation) and obstacles become forbidden regions. On a low-dimensional mobile base you grid that space and run $\\mathbf{A}^*$, which is just Dijkstra with an admissible heuristic that aims the search at the goal. When the dimension climbs and the grid detonates under the **curse of dimensionality**, you stop enumerating and start sampling: **PRM** to build a reusable roadmap for many queries, **RRT** to grow a single tree fast (and respect nonholonomic constraints), $\\mathbf{RRT}^*$ to rewire that tree toward the optimal path. Then trajectory optimization (**CHOMP**, **TrajOpt**) turns the geometric squiggle into a smooth, timed, limit-respecting trajectory the controller can track. And in the real, moving world, you split the job: a **global** planner for the route, a fast **local** planner (**DWA**) for the surprises.",
